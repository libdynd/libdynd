...
TEST(DTypeDType, StructRefCount) {
    ...
    // Single CStruct Instance
    a = nd::empty("{dt: dtype; more: {a: int32; b: dtype}; other: string}")(0 <= irange() < 2);
    ...
    a = nd::empty(10, "M, {dt: dtype; more: {a: int32; b: dtype}; other: string}")(irange(), 0 <= irange() < 2);
