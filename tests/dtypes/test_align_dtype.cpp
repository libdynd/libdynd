...
TEST(AlignDType, Create) {
    ...
    EXPECT_EQ(d.storage_type(), make_fixedbytes_dtype(4, 1));
...
TEST(AlignDType, Chained) {
    ...
    ndt::type dt = make_unaligned_dtype(make_byteswap_dtype<int>());
    EXPECT_EQ(make_byteswap_dtype(ndt::make_dtype<int>(), make_view_dtype(make_fixedbytes_dtype(4, 4), make_fixedbytes_dtype(4, 1))), dt);
    EXPECT_EQ(make_fixedbytes_dtype(4, 1), dt.storage_type());
