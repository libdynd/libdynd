...
TEST(StructDType, Basic) {
    EXPECT_NE(make_struct_type(ndt::make_dtype<int>(), "x"),
                    make_struct_type(ndt::make_dtype<int>(), "y"));
    EXPECT_NE(make_struct_type(ndt::make_dtype<float>(), "x"),
                    make_struct_type(ndt::make_dtype<int>(), "x"));
}
...
TEST(StructDType, CreateOneField) {
    ...
    dt = make_struct_type(ndt::make_dtype<int32_t>(), "x");
...
TEST(StructDType, CreateTwoField) {
    ...
    dt = make_struct_type(ndt::make_dtype<int64_t>(), "a", ndt::make_dtype<int32_t>(), "b");
    EXPECT_EQ(struct_type_id, dt.get_type_id());
...
TEST(StructDType, CreateThreeField) {
    ...
    dt = make_struct_type(d1, "x", d2, "y", d3, "z");
    EXPECT_EQ(struct_type_id, dt.get_type_id());
    EXPECT_EQ(0u, dt.get_data_size());
...
TEST(StructDType, CanonicalDType) {
    ...
    dt = make_struct_type(d1, "x", d2, "y", d3, "z");
    EXPECT_EQ(make_struct_type(ndt::make_dtype<std::complex<double> >(), "x",
...
TEST(StructDType, IsExpression) {
    ...
    ndt::type d2 = make_byteswap_type<int32_t>();
    ndt::type d3 = make_fixedstring_type(5, string_encoding_utf_32);
    ndt::type d = make_struct_type(d1, "x", d2, "y", d3, "z");
...
TEST(StructDType, DifferentDTypeAssign) {
    ...
    ndt::type dt2 = make_struct_type(ndt::make_dtype<float>(), "y", ndt::make_dtype<int>(), "z", ndt::make_dtype<uint8_t>(), "x");
...
TEST(StructDType, SingleCompare) {
    nd::array a, b;
    ndt::type sdt = make_struct_type(ndt::make_dtype<int32_t>(), "a",
...
TEST(StructDType, SingleCompareDifferentMetadata) {
    ...
    ndt::type sdt = make_struct_type(ndt::make_dtype<int32_t>(), "a",
                    ndt::make_dtype<float>(), "b", ndt::make_dtype<int64_t>(), "c");
    ndt::type sdt_reverse = make_struct_type(ndt::make_dtype<int64_t>(), "c",
