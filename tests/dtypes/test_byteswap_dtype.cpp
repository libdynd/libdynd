...
TEST(ByteswapDType, Create) {
    ...
    EXPECT_EQ(d.storage_type(), make_fixedbytes_dtype(4, 4));
    ...
    // The storage is the a bytes dtype with matching storage and alignment
    EXPECT_EQ(d.storage_type(), make_fixedbytes_dtype(16, scalar_align_of<complex<double> >::value));
...
TEST(ByteswapDType, Basic) {
    ...
    a = nd::make_pod_array(make_byteswap_dtype<int16_t>(), (char *)&value16);
    EXPECT_EQ(0x6213, a.as<int16_t>());

    int32_t value32 = 0x12345678;
    a = nd::make_pod_array(make_byteswap_dtype<int32_t>(), (char *)&value32);
    EXPECT_EQ(0x78563412, a.as<int32_t>());

    int64_t value64 = 0x12345678abcdef01LL;
    a = nd::make_pod_array(make_byteswap_dtype<int64_t>(), (char *)&value64);
    EXPECT_EQ(0x01efcdab78563412LL, a.as<int64_t>());

    value32 = 0xDA0F4940;
    a = nd::make_pod_array(make_byteswap_dtype<float>(), (char *)&value32);
    EXPECT_EQ(3.1415926f, a.as<float>());

    value64 = 0x112D4454FB210940LL;
    a = nd::make_pod_array(make_byteswap_dtype<double>(), (char *)&value64);
    EXPECT_EQ(3.14159265358979, a.as<double>());
    a = a.eval();
    EXPECT_EQ(3.14159265358979, a.as<double>());

    uint32_t value32_pair[2] = {0xDA0F4940, 0xC1B88FD3};
    a = nd::make_pod_array(make_byteswap_dtype<complex<float> >(), (char *)&value32_pair);
    ...
    a = nd::make_pod_array(make_byteswap_dtype<complex<double> >(), (char *)&value64_pair);
...
TEST(ByteswapDType, CanonicalDType) {
    ...
    EXPECT_EQ((ndt::make_dtype<float>()), (make_byteswap_dtype<float>().get_canonical_type()));
}
