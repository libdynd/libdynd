...
TEST(AlignDType, Create) {
    ...
    d = ndt::make_unaligned_type<float>();
    ...
    //EXPECT_THROW(d = make_unaligned_type([[some object dtype]]), runtime_error);
}
...
TEST(AlignDType, Chained) {
    ...
    ndt::type dt = make_unaligned_type(make_byteswap_type<int>());
...
TEST(AlignDType, CanonicalDType) {
    ...
    EXPECT_EQ((ndt::make_dtype<float>()), (ndt::make_unaligned_type<float>().get_canonical_type()));
}
