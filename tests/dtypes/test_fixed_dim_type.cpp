...
TEST(FixedDimDType, Create) {
    ...
    d = ndt::make_fixed_dim(3, ndt::make_dtype<int32_t>());
    ...
    d = ndt::make_fixed_dim(1, ndt::make_dtype<int32_t>());
    EXPECT_EQ(fixed_dim_type_id, d.get_type_id());
    EXPECT_EQ(uniform_dim_kind, d.get_kind());
    EXPECT_EQ(4u, d.get_data_alignment());
...
TEST(FixedDimDType, CreateCOrder) {
    ...
    ndt::type d = ndt::make_fixed_dim(3, shape, ndt::make_dtype<int16_t>(), NULL);
    EXPECT_EQ(fixed_dim_type_id, d.get_type_id());
    EXPECT_EQ(ndt::make_fixed_dim(2, shape+1, ndt::make_dtype<int16_t>(), NULL), d.at(0));
    EXPECT_EQ(ndt::make_fixed_dim(1, shape+2, ndt::make_dtype<int16_t>(), NULL), d.at(0,0));
...
TEST(FixedDimDType, CreateFOrder) {
    ...
    intptr_t shape[3] = {2, 3, 4};
    ndt::type d = ndt::make_fixed_dim(3, shape, ndt::make_dtype<int16_t>(), axis_perm);
...
TEST(FixedDimDType, Basic) {
    ...
    a = nd::empty(ndt::make_fixed_dim(3, ndt::make_dtype<float>()));
    a.vals() = vals;

    EXPECT_EQ(ndt::make_fixed_dim(3, ndt::make_dtype<float>()), a.get_dtype());
...
TEST(FixedDimDType, SimpleIndex) {
    ...
    ASSERT_EQ(ndt::make_fixed_dim(2,
                    ndt::make_fixed_dim(3, ndt::make_dtype<int16_t>())),
                a.get_dtype());

    nd::array b;

    b = a(0);
    ASSERT_EQ(ndt::make_fixed_dim(3, ndt::make_dtype<int16_t>()),
                b.get_dtype());
