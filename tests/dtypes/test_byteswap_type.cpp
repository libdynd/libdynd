...
TEST(ByteswapDType, Create) {
    ...
    EXPECT_EQ(d.storage_type(), make_fixedbytes_type(4, 4));
    EXPECT_TRUE(d.is_expression());

    d = make_byteswap_type<complex<double> >();
    // The value has the native byte-order dtype
    EXPECT_EQ(d.value_type(), ndt::make_dtype<complex<double> >());
    // The storage is the a bytes dtype with matching storage and alignment
    EXPECT_EQ(d.storage_type(), make_fixedbytes_type(16, scalar_align_of<complex<double> >::value));

    // Only basic built-in dtypes can be used to make a byteswap dtype
    EXPECT_THROW(d = make_byteswap_type(make_convert_type<int, float>()), runtime_error);
}
...
TEST(ByteswapDType, Basic) {
    ...
    a = nd::make_pod_array(make_byteswap_type<int16_t>(), (char *)&value16);
    EXPECT_EQ(0x6213, a.as<int16_t>());

    int32_t value32 = 0x12345678;
    a = nd::make_pod_array(make_byteswap_type<int32_t>(), (char *)&value32);
    EXPECT_EQ(0x78563412, a.as<int32_t>());

    int64_t value64 = 0x12345678abcdef01LL;
    a = nd::make_pod_array(make_byteswap_type<int64_t>(), (char *)&value64);
    EXPECT_EQ(0x01efcdab78563412LL, a.as<int64_t>());

    value32 = 0xDA0F4940;
    a = nd::make_pod_array(make_byteswap_type<float>(), (char *)&value32);
    EXPECT_EQ(3.1415926f, a.as<float>());

    value64 = 0x112D4454FB210940LL;
    a = nd::make_pod_array(make_byteswap_type<double>(), (char *)&value64);
    EXPECT_EQ(3.14159265358979, a.as<double>());
    a = a.eval();
    EXPECT_EQ(3.14159265358979, a.as<double>());

    uint32_t value32_pair[2] = {0xDA0F4940, 0xC1B88FD3};
    a = nd::make_pod_array(make_byteswap_type<complex<float> >(), (char *)&value32_pair);
    ...
    a = nd::make_pod_array(make_byteswap_type<complex<double> >(), (char *)&value64_pair);
...
TEST(ByteswapDType, CanonicalDType) {
    ...
    EXPECT_EQ((ndt::make_dtype<float>()), (make_byteswap_type<float>().get_canonical_type()));
}
