                                                    ...
                                                    const dynd::nd::array &val2)
{
  ...
  if (val1.get_type().get_type_id() == cuda_device_type_id &&
      val2.get_type().get_type_id() == cuda_device_type_id) {
  ...
  if (val1.equals_exact(val2)) {
    ...
    if (val1.get_type() != val2.get_type()) {
      ...
      const ndt::base_struct_type *bsd =
          val1.get_type().extended<ndt::base_struct_type>();
...
inline ::testing::AssertionResult
CompareDyNDArrayToJSON(const char *expr1, const char *expr2, const char *json,
...
AssertRelErrorLE(const char *DYND_UNUSED(expected_expr),
                 const char *DYND_UNUSED(actual_expr),
                 const char *DYND_UNUSED(rel_error_max_expr), T expected,
                 T actual, float rel_error_max)
{
...
inline ::testing::AssertionResult
AssertRelErrorLE(const char *expected_expr, const char *actual_expr,
                 const char *rel_error_max_expr, float expected,
                 dynd::complex<float> actual, float rel_error_max)
{
  return AssertRelErrorLE(expected_expr, actual_expr, rel_error_max_expr,
                          dynd::complex<float>(expected), actual,
                          rel_error_max);
}
...
{
  return AssertRelErrorLE(expected_expr, actual_expr, rel_error_max_expr,
                          dynd::complex<double>(expected), actual,
                          rel_error_max);
}
...
struct test_class {
  ...
  test_class(double rel_error_max) : rel_error_max(rel_error_max)
  {
    flag = true;
  }
