...
TEST(DataShapeFormatter, ArrayBuiltinAtoms)
{
  ...
  EXPECT_EQ("float16",
            format_datashape(nd::array(float16(0.f, assign_error_nocheck)), "",
                             false));
  EXPECT_EQ("float32", format_datashape(nd::array(0.f), "", false));
  EXPECT_EQ("float64", format_datashape(nd::array(0.), "", false));
  EXPECT_EQ("complex[float32]",
            format_datashape(nd::array(dynd::complex<float>(0.f)), "", false));
  EXPECT_EQ("complex[float64]",
            format_datashape(nd::array(dynd::complex<double>(0.)), "", false));
}
...
TEST(DataShapeFormatter, DTypeBuiltinAtoms)
{
  ...
  EXPECT_EQ(
      "complex[float32]",
      format_datashape(ndt::type::make<dynd::complex<float>>(), "", false));
  EXPECT_EQ(
      "complex[float64]",
      format_datashape(ndt::type::make<dynd::complex<double>>(), "", false));
}
...
TEST(DataShapeFormatter, ArrayStringAtoms)
{
  ...
  EXPECT_EQ(
      "string",
      format_datashape(nd::empty(ndt::string_type::make(string_encoding_utf_8)),
                       "", false));
  EXPECT_EQ(
      "string",
      format_datashape(nd::empty(ndt::string_type::make(string_encoding_ascii)),
                       "", false));
  EXPECT_EQ("string", format_datashape(nd::empty(ndt::string_type::make(
  ...
  EXPECT_EQ(
      "string",
      format_datashape(nd::empty(ndt::string_type::make(string_encoding_ucs_2)),
                       "", false));
  EXPECT_EQ("string", format_datashape(nd::empty(ndt::fixed_string_type::make(
                                           1, string_encoding_utf_8)),
                                       "", false));
  EXPECT_EQ("string", format_datashape(nd::empty(ndt::fixed_string_type::make(
                                           10, string_encoding_utf_8)),
                                       "", false));
  EXPECT_EQ("string", format_datashape(nd::empty(ndt::fixed_string_type::make(
                                           10, string_encoding_ascii)),
                                       "", false));
  EXPECT_EQ("string", format_datashape(nd::empty(ndt::fixed_string_type::make(
                                           10, string_encoding_utf_16)),
                                       "", false));
  EXPECT_EQ("string", format_datashape(nd::empty(ndt::fixed_string_type::make(
                                           10, string_encoding_utf_32)),
                                       "", false));
  EXPECT_EQ("string", format_datashape(nd::empty(ndt::fixed_string_type::make(
                                           10, string_encoding_ucs_2)),
                                       "", false));
}
...
TEST(DataShapeFormatter, DTypeStringAtoms)
{
  ...
  EXPECT_EQ("string",
            format_datashape(ndt::string_type::make(string_encoding_utf_8), "",
                             false));
  EXPECT_EQ("string",
            format_datashape(ndt::string_type::make(string_encoding_ascii), "",
                             false));
  EXPECT_EQ("string",
            format_datashape(ndt::string_type::make(string_encoding_utf_16), "",
                             false));
  EXPECT_EQ("string",
            format_datashape(ndt::string_type::make(string_encoding_utf_32), "",
                             false));
  EXPECT_EQ("string",
            format_datashape(ndt::string_type::make(string_encoding_ucs_2), "",
                             false));
  EXPECT_EQ("string", format_datashape(ndt::fixed_string_type::make(
                                           1, string_encoding_utf_8),
                                       "", false));
  EXPECT_EQ("string", format_datashape(ndt::fixed_string_type::make(
                                           10, string_encoding_utf_8),
...
TEST(DataShapeFormatter, ArrayUniformArrays)
{
  ...
  EXPECT_EQ("var * int32", format_datashape(nd::empty(ndt::var_dim_type::make(
                                                ndt::type::make<int32_t>())),
                                            "", false));
  EXPECT_EQ(
      "var * 3 * int32",
      format_datashape(nd::empty(ndt::var_dim_type::make(
                           ndt::make_fixed_dim(3, ndt::type::make<int32_t>()))),
                       "", false));
}
...
TEST(DataShapeFormatter, DTypeUniformArrays)
{
  EXPECT_EQ(
      ...
      format_datashape(ndt::make_fixed_dim_kind(ndt::type::make<int32_t>(), 3),
                       "", false));
  EXPECT_EQ("var * int32", format_datashape(ndt::var_dim_type::make(
                                                ndt::type::make<int32_t>()),
                                            "", false));
  EXPECT_EQ("var * 3 * int32",
            format_datashape(ndt::var_dim_type::make(ndt::make_fixed_dim(
                                 3, ndt::type::make<int32_t>())),
                             "", false));
  EXPECT_EQ("var * Fixed * int32",
            format_datashape(ndt::var_dim_type::make(ndt::make_fixed_dim_kind(
                                 ndt::type::make<int32_t>())),
                             "", false));
}
...
TEST(DataShapeFormatter, ArrayStructs)
{
  EXPECT_EQ("{x: int32, y: float64}",
            format_datashape(nd::empty(ndt::struct_type::make(
                                 {"x", "y"}, {ndt::type::make<int32_t>(),
                                              ndt::type::make<double>()})),
                             "", false));
  EXPECT_EQ("{x: var * {a: int32, b: int8}, y: 5 * var * uint8}",
            format_datashape(
                nd::empty(ndt::struct_type::make(
                    {"x", "y"},
                    {ndt::var_dim_type::make(ndt::struct_type::make(
                         {"a", "b"}, {ndt::type::make<int32_t>(),
                                      ndt::type::make<int8_t>()})),
                     ndt::make_fixed_dim(5, ndt::var_dim_type::make(
...
TEST(DataShapeFormatter, DTypeStructs)
{
  EXPECT_EQ("{x: int32, y: float64}",
            format_datashape(
                ndt::struct_type::make({"x", "y"}, {ndt::type::make<int32_t>(),
                                                    ndt::type::make<double>()}),
                "", false));
  EXPECT_EQ("{x: var * {a: int32, b: int8}, y: 5 * var * uint8}",
            format_datashape(
  ...
  EXPECT_EQ(
      ...
      format_datashape(
          ndt::struct_type::make(
              {"x", "y"}, {ndt::make_fixed_dim(
                               7, ndt::struct_type::make(
                                      {"a", "b"}, {ndt::type::make<int32_t>(),
                                                   ndt::type::make<int8_t>()})),
                           ndt::var_dim_type::make(ndt::make_fixed_dim(
                               4, ndt::type::make<uint8_t>()))}),
          "", false));
}