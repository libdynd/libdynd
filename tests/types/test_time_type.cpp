...
TEST(TimeDType, ValueCreationAbstract)
{
  ...
  EXPECT_EQ(12 * DYND_TICKS_PER_HOUR + 34 * DYND_TICKS_PER_MINUTE + 56 * DYND_TICKS_PER_SECOND +
                700 * DYND_TICKS_PER_MILLISECOND,
            nd::array("12:34:56.7").ucast(d).view_scalars(di).as<int64_t>());
  EXPECT_EQ(12 * DYND_TICKS_PER_HOUR + 34 * DYND_TICKS_PER_MINUTE + 56 * DYND_TICKS_PER_SECOND + 7890123,
            nd::array("12:34:56.7890123").ucast(d).view_scalars(di).as<int64_t>());
  EXPECT_EQ(DYND_TICKS_PER_DAY - 1, nd::array("23:59:59.9999999").ucast(d).view_scalars(di).as<int64_t>());
}

TEST(TimeDType, ConvertToString)
{
  EXPECT_EQ("00:00", nd::array("00:00:00.000").cast(ndt::type("time")).as<std::string>());
  EXPECT_EQ("00:00", nd::array("12:00:00.000 AM").cast(ndt::type("time")).as<std::string>());
  EXPECT_EQ("12:00", nd::array("12:00:00.000 PM").cast(ndt::type("time")).as<std::string>());
  EXPECT_EQ("23:59:59.9999999", nd::array("23:59:59.9999999").cast(ndt::type("time")).as<std::string>());
  EXPECT_EQ("00:00:01", nd::array("00:00:01.000").cast(ndt::type("time")).as<std::string>());
  EXPECT_EQ("00:00:00.0000001", nd::array("00:00:00.0000001").cast(ndt::type("time")).as<std::string>());
  EXPECT_EQ("12:34:56.7", nd::array("12:34:56.700").cast(ndt::type("time")).as<std::string>());
  EXPECT_EQ("23:59:56.78", nd::array("23:59:56.7800").cast(ndt::type("time")).as<std::string>());
  EXPECT_EQ("23:59:59.789", nd::array("23:59:59.78900").cast(ndt::type("time")).as<std::string>());
  EXPECT_EQ("23:59:59.78901", nd::array("23:59:59.789010").cast(ndt::type("time")).as<std::string>());
  EXPECT_EQ("12:34:56.7890123", nd::array("12:34:56.7890123").cast(ndt::type("time")).as<std::string>());
}
