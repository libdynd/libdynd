...
TEST(FixedDimType, SimpleIndex)
{
  ...
  ASSERT_EQ(
      ndt::make_fixed_dim(2, ndt::make_fixed_dim(3, ndt::make_type<int16_t>())),
      a.get_type());
...
TEST(FixedDimType, IsTypeSubarray)
{
  ...
  EXPECT_TRUE(
      ndt::type("5 * int32").is_type_subarray(ndt::make_type<int32_t>()));
  EXPECT_FALSE(
      ndt::make_type<int32_t>().is_type_subarray(ndt::type("5 * int32")));
  EXPECT_FALSE(
