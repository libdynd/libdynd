...
TEST(AdaptType, Byteswap) {
  ndt::type tp = ndt::make_type<ndt::adapt_type>(ndt::make_type<float>(),
                                                 ndt::make_type<ndt::fixed_bytes_type>(sizeof(float), alignof(float)),
  ...
  EXPECT_EQ(ndt::make_type<ndt::fixed_bytes_type>(sizeof(float), alignof(float)), tp.storage_type());
  ...
  tp = ndt::make_type<ndt::adapt_type>(
      ...
      ndt::make_type<ndt::fixed_bytes_type>(sizeof(dynd::complex<double>), alignof(dynd::complex<double>)),
      nd::pairwise_byteswap, nd::pairwise_byteswap);
  // The value has the native byte-order type
  EXPECT_EQ(ndt::make_type<dynd::complex<double>>(), tp.value_type());
  // The storage is the a bytes type with matching storage and alignment
  EXPECT_EQ(ndt::make_type<ndt::fixed_bytes_type>(sizeof(dynd::complex<double>), alignof(dynd::complex<double>)),
            tp.storage_type());
  EXPECT_TRUE(tp.is_expression());
}
...
TEST(AdaptType, ByteswapEval) {
  nd::array a = nd::empty(ndt::make_type<ndt::adapt_type>(
      ndt::make_type<int16_t>(), ndt::make_type<ndt::fixed_bytes_type>(sizeof(int16_t), alignof(int16_t)), nd::byteswap,
      nd::byteswap));
  a.assign(0x1362);
  ...
  a = nd::empty(ndt::make_type<ndt::adapt_type>(
      ndt::make_type<int64_t>(), ndt::make_type<ndt::fixed_bytes_type>(sizeof(int64_t), alignof(int64_t)), nd::byteswap,
      nd::byteswap));
  ...
  a = nd::empty(ndt::make_type<ndt::adapt_type>(ndt::make_type<double>(),
                                                ndt::make_type<ndt::fixed_bytes_type>(sizeof(double), alignof(double)),
                                                nd::byteswap, nd::byteswap));
  ...
  a = nd::empty(ndt::make_type<ndt::adapt_type>(
      ...
      ndt::make_type<ndt::fixed_bytes_type>(sizeof(dynd::complex<float>), alignof(dynd::complex<float>)),
      nd::pairwise_byteswap, nd::pairwise_byteswap));
