...
TEST(Complex, Math) {
    ...
    EXPECT_DOUBLE_EQ(abs(z), abs(z.as<complex>()));
    EXPECT_DOUBLE_EQ(arg(z), arg(z.as<complex>()));
    EXPECT_COMPLEX_DOUBLE_EQ(exp(z), exp(z.as<complex>()));
    EXPECT_COMPLEX_DOUBLE_EQ(log(z), log(z.as<complex>()));
    EXPECT_COMPLEX_DOUBLE_EQ(sqrt(z), sqrt(z.as<complex>()));
    EXPECT_COMPLEX_DOUBLE_EQ(pow(z, 1.0), pow(z.as<complex>(), 1.0));
    EXPECT_COMPLEX_DOUBLE_EQ(pow(z, 2.0), pow(z.as<complex>(), 2.0));
    EXPECT_COMPLEX_DOUBLE_EQ(pow(z, 3.0), pow(z.as<complex>(), 3.0));
    EXPECT_COMPLEX_DOUBLE_EQ(pow(z, 7.4), pow(z.as<complex>(), 7.4));
    ...
    EXPECT_COMPLEX_DOUBLE_EQ(log(z), log(z.as<complex>()));
    EXPECT_COMPLEX_DOUBLE_EQ(sqrt(z), sqrt(z.as<complex>()));
    EXPECT_COMPLEX_DOUBLE_EQ(pow(z, 0.0), pow(z.as<complex>(), 0.0));
    EXPECT_COMPLEX_DOUBLE_EQ(pow(z, 1.0), pow(z.as<complex>(), 1.0));
    EXPECT_COMPLEX_DOUBLE_EQ(pow(z, 2.0), pow(z.as<complex>(), 2.0));
    EXPECT_COMPLEX_DOUBLE_EQ(pow(z, 3.0), pow(z.as<complex>(), 3.0));
    EXPECT_COMPLEX_DOUBLE_EQ(pow(z, 7.4), pow(z.as<complex>(), 7.4));
    EXPECT_COMPLEX_DOUBLE_EQ(pow(z, dynd_complex<double>(0.0, 1.0)), pow(z.as<complex>(), complex<double>(0.0, 1.0)));
    EXPECT_COMPLEX_DOUBLE_EQ(pow(z, dynd_complex<double>(0.0, -1.0)), pow(z.as<complex>(), complex<double>(0.0, -1.0)));
    EXPECT_COMPLEX_DOUBLE_EQ(pow(z, dynd_complex<double>(7.4, -6.3)), pow(z.as<complex>(), complex<double>(7.4, -6.3)));
    ...
    EXPECT_COMPLEX_DOUBLE_EQ(log(z), log(z.as<complex>()));
    EXPECT_COMPLEX_DOUBLE_EQ(sqrt(z), sqrt(z.as<complex>()));
    EXPECT_COMPLEX_DOUBLE_EQ(cos(z), cos(z.as<complex>()));
    EXPECT_COMPLEX_DOUBLE_EQ(sin(z), sin(z.as<complex>()));
