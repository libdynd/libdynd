...
class RefResWrapper {
public:
    void mem_func_0(int &res, int x, int y) const {
        func_ref_res_0(res, x, y);
    }

    void mem_func_1(int &res, const int (&x)[3]) const {
        func_ref_res_1(res, x);
    }

    void mem_func_2(int &res, const int (&x)[3], const int (&y)[3]) const {
        func_ref_res_2(res, x, y);
    }

    void mem_func_3(int &res, const int(&x)[2][3]) const {
        func_ref_res_3(res, x);
    }

    void mem_func_4(int (&res)[2], int x, int y) const {
        func_ref_res_4(res, x, y);
    }

    void mem_func_5(int (&res)[3], const int(&x)[3][3], const int(&y)[3]) const {
        func_ref_res_5(res, x, y);
    }

    void mem_func_6(double (&res)[2][2], int x) const {
        func_ref_res_6(res, x);
    }

    void mem_func_7(int (&res)[3][3], const int(&x)[3][3], const int(&y)[3][3]) const {
        func_ref_res_7(res, x, y);
    }
};
...
TEST(ArrayViews, IntMemFunc) {
    ...
    res = nd::elwise(obj, &RefResWrapper::mem_func_0, a, b);
    EXPECT_EQ(-20, res.as<int>());

    res = nd::elwise(obj, &RefResWrapper::mem_func_4, a, b);
    EXPECT_EQ(ndt::type("2 * int32"), res.get_type());
    EXPECT_EQ(20, res(0).as<int>());
    EXPECT_EQ(10, res(1).as<int>());
    ...
    res = nd::elwise(obj, &RefResWrapper::mem_func_6, a);
    EXPECT_EQ(ndt::type("2 * 2 * float64"), res.get_type());
    EXPECT_EQ(cos((double) 1), res(0,0).as<double>());
    EXPECT_EQ(-sin((double) 1), res(0,1).as<double>());
    EXPECT_EQ(sin((double) 1), res(1,0).as<double>());
    ...
    a.vals() = vals_1[0];
    res = nd::elwise(obj, &RefResWrapper::mem_func_1, a);
    EXPECT_EQ(ndt::type("int32"), res.get_type());
    EXPECT_EQ(3, res.as<int>());
    ...
    a.vals() = vals_1[0];
    b.vals() = vals_1[1];
    res = nd::elwise(obj, &RefResWrapper::mem_func_2, a, b);
    EXPECT_EQ(ndt::type("int32"), res.get_type());
    EXPECT_EQ(14, res.as<int>());
    ...
    a.vals() = vals_1;
    res = nd::elwise(obj, &RefResWrapper::mem_func_3, a);
    EXPECT_EQ(ndt::type("int32"), res.get_type());
    EXPECT_EQ(6, res.as<int>());
    ...
    a.vals() = avals_2;
    b.vals() = bvals_2;
    res = nd::elwise(obj, &RefResWrapper::mem_func_5, a, b);
    EXPECT_EQ(ndt::type("3 * int32"), res.get_type());
    for (int i = 0; i < 3; ++i) {
            EXPECT_EQ(res(i), avals_2[i][0] * bvals_2[0] + avals_2[i][1] * bvals_2[1]
                + avals_2[i][2] * bvals_2[2]);
    ...
    b.vals() = bvals_3;
    res = nd::elwise(obj, &RefResWrapper::mem_func_7, a, b);
    EXPECT_EQ(ndt::type("3 * 3 * int32"), res.get_type());
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            EXPECT_EQ(res(i, j), avals_2[i][0] * bvals_3[0][j] + avals_2[i][1] * bvals_3[1][j]
...
class RefResCallable6 {
public:
    void operator ()(double (&res)[2][2], int x) const {
        func_ref_res_6(res, x);
    }
};

class RefResCallable7 {
public:
    void operator ()(int (&res)[3][3], const int(&x)[3][3], const int(&y)[3][3]) const {
        func_ref_res_7(res, x, y);
    }
};

TEST(ArrayViews, IntCallable) {
    RefResWrapper obj;
    ...
    res = nd::elwise(RefResCallable6(), a);
    EXPECT_EQ(ndt::type("2 * 2 * float64"), res.get_type());
    EXPECT_EQ(cos((double) 1), res(0,0).as<double>());
    EXPECT_EQ(-sin((double) 1), res(0,1).as<double>());
    EXPECT_EQ(sin((double) 1), res(1,0).as<double>());
    ...
    b.vals() = bvals_3;
    res = nd::elwise(RefResCallable7(), a, b);
    EXPECT_EQ(ndt::type("3 * 3 * int32"), res.get_type());
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            EXPECT_EQ(res(i, j), avals_2[i][0] * bvals_3[0][j] + avals_2[i][1] * bvals_3[1][j]
