...
TEST(ArrayViews, OneDimensionalRawMemory) {
    ...
    EXPECT_EQ(make_strided_dim_dtype(ndt::make_dtype<uint64_t>()), b.get_dtype());
    ...
    EXPECT_EQ(make_strided_dim_dtype(make_view_dtype(ndt::make_dtype<uint64_t>(), make_fixedbytes_dtype(8, 1))),
                    b.get_dtype());
    EXPECT_EQ(1u, b.get_shape().size());
...
TEST(ArrayViews, MultiDimensionalRawMemory) {
    ...
    EXPECT_EQ(make_strided_dim_dtype(ndt::make_dtype<int32_t>(), 2), b.get_dtype());
...
TEST(ArrayViews, ExpressionDType) {
    ...
    EXPECT_EQ(make_strided_dim_dtype(make_view_dtype(ndt::make_dtype<int16_t>(), make_convert_dtype<uint16_t, uint32_t>()), 2),
                    b.get_dtype());
    EXPECT_EQ(2u, b.get_shape().size());
    EXPECT_EQ(2, b.get_shape()[0]);
    EXPECT_EQ(3, b.get_shape()[1]);
