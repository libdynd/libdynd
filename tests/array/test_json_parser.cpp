...
TEST(JSONParser, String) {
    ...
    EXPECT_EQ(ndt::make_string(string_encoding_utf_8), n.get_dtype());
    ...
    EXPECT_EQ(ndt::make_string(string_encoding_utf_16), n.get_dtype());
    EXPECT_EQ("testing one two three", n.as<string>());
...
TEST(JSONParser, ListBools) {
    ...
    n = parse_json(ndt::make_var_dim(ndt::make_dtype<dynd_bool>()),
                    "  [true, true, false, null]  ");
    EXPECT_EQ(ndt::make_var_dim(ndt::make_dtype<dynd_bool>()), n.get_dtype());
    EXPECT_TRUE(n(0).as<bool>());
    EXPECT_TRUE(n(1).as<bool>());
    EXPECT_FALSE(n(2).as<bool>());
    EXPECT_FALSE(n(3).as<bool>());

    n = parse_json(ndt::make_fixed_dim(4, ndt::make_dtype<dynd_bool>()),
                    "  [true, true, false, null]  ");
    EXPECT_EQ(ndt::make_fixed_dim(4, ndt::make_dtype<dynd_bool>()), n.get_dtype());
    EXPECT_TRUE(n(0).as<bool>());
    EXPECT_TRUE(n(1).as<bool>());
    EXPECT_FALSE(n(2).as<bool>());
    EXPECT_FALSE(n(3).as<bool>());

    EXPECT_THROW(parse_json(ndt::make_var_dim(ndt::make_dtype<dynd_bool>()),
                    "[true, true, false, null] 3.5"),
                    runtime_error);
    EXPECT_THROW(parse_json(ndt::make_fixed_dim(4, ndt::make_dtype<dynd_bool>()),
                    "[true, true, false, null] 3.5"),
                    runtime_error);
    EXPECT_THROW(parse_json(ndt::make_fixed_dim(3, ndt::make_dtype<dynd_bool>()),
                    "[true, true, false, null]"),
                    runtime_error);
    EXPECT_THROW(parse_json(ndt::make_fixed_dim(5, ndt::make_dtype<dynd_bool>()),
                    "[true, true, false, null]"),
                    runtime_error);
}
...
TEST(JSONParser, NestedListInts) {
    ...
    n = parse_json(ndt::make_fixed_dim(3, ndt::make_var_dim(ndt::make_dtype<int>())),
                    "  [[1,2,3], [4,5], [6,7,-10,1000] ]  ");
    EXPECT_EQ(ndt::make_fixed_dim(3, ndt::make_var_dim(ndt::make_dtype<int>())), n.get_dtype());
    ...
    n = parse_json(ndt::make_var_dim(ndt::make_fixed_dim(3, ndt::make_dtype<int>())),
                    "  [[1,2,3], [4,5,2] ]  ");
    EXPECT_EQ(ndt::make_var_dim(ndt::make_fixed_dim(3, ndt::make_dtype<int>())), n.get_dtype());
    EXPECT_EQ(1, n(0,0).as<int>());
    EXPECT_EQ(2, n(0,1).as<int>());
    EXPECT_EQ(3, n(0,2).as<int>());
    EXPECT_EQ(4, n(1,0).as<int>());
...
TEST(JSONParser, Struct) {
    nd::array n;
    ndt::type sdt = ndt::make_cstruct(ndt::make_dtype<int>(), "id", ndt::make_dtype<double>(), "amount",
...
TEST(JSONParser, NestedStruct) {
    nd::array n;
    ndt::type sdt = ndt::make_cstruct(ndt::make_fixed_dim(3, ndt::make_dtype<float>()), "position",
...
TEST(JSONParser, ListOfStruct) {
    nd::array n;
    ndt::type sdt = ndt::make_var_dim(ndt::make_cstruct(ndt::make_fixed_dim(3, ndt::make_dtype<float>()), "position",
                    ndt::make_dtype<double>(), "amount",
                    ndt::make_cstruct(ndt::make_string(), "name", ndt::make_date(), "when"), "data"));
...
TEST(JSONParser, JSONDType) {
    ...
    EXPECT_EQ(ndt::make_json(), n.get_dtype());
    ...
    EXPECT_EQ(ndt::make_cstruct(ndt::make_json(), "a", ndt::make_dtype<int32_t>(), "b", ndt::make_string(), "c"),
                    n.get_dtype());
