...
TEST(ArrayAssign, ScalarAssignment_Int8) {
    ...
    EXPECT_THROW(a.val_assign(-129), overflow_error);
    EXPECT_THROW(a.val_assign(128), overflow_error);
    a.val_assign(5.0);
    EXPECT_EQ(5, *ptr_i8);
    a.val_assign(-100.0f);
    EXPECT_EQ(-100, *ptr_i8);
    EXPECT_THROW(a.val_assign(1.25), runtime_error);
    EXPECT_THROW(a.val_assign(128.0), overflow_error);
    ...
    EXPECT_EQ(1, *ptr_i8);
    EXPECT_THROW(a.val_assign(-129.0, assign_error_overflow), overflow_error);
...
TEST(ArrayAssign, ScalarAssignment_UInt16) {
    nd::array a;
    ...
    a.val_assign(false);
    EXPECT_EQ(0, *ptr_u16);
    EXPECT_THROW(a.val_assign(-1), overflow_error);
    EXPECT_THROW(a.val_assign(-1, assign_error_overflow), overflow_error);
...
TEST(ArrayAssign, ScalarAssignment_Float32) {
    nd::array a;
    const float *ptr_f32;
...
TEST(ArrayAssign, ScalarAssignment_Float64) {
    nd::array a;
...
TEST(ArrayAssign, ScalarAssignment_Uint64) {
    nd::array a;
...
TEST(ArrayAssign, ScalarAssignment_Uint64_LargeNumbers) {
    nd::array a;
    const uint64_t *ptr_u64;
...
TEST(ArrayAssign, ChainedCastingRead) {
    ...
    EXPECT_EQ(make_strided_dim_type(
                    make_convert_type(ndt::make_dtype<float>(),
                                    make_convert_type<int, float>(assign_error_overflow), assign_error_inexact)),
              b.get_dtype());

    // Evaluating the values should truncate them to integers
    b = b.eval();
    // Now it's just the value dtype, no chaining
    EXPECT_EQ(make_strided_dim_type(ndt::make_dtype<float>()), b.get_dtype());
    ...
    EXPECT_EQ(make_strided_dim_type(
                make_convert_type(ndt::make_dtype<int32_t>(),
                    make_convert_type(ndt::make_dtype<float>(),
                        make_convert_type(ndt::make_dtype<int64_t>(),
                            make_convert_type(ndt::make_dtype<int16_t>(),
                                make_convert_type(ndt::make_dtype<int32_t>(),
                                    make_convert_type<int16_t, float>(
            ...
            b.get_dtype());
    b = b.eval();
    EXPECT_EQ(make_strided_dim_type(ndt::make_dtype<int32_t>()), b.get_dtype());
...
TEST(ArrayAssign, ChainedCastingWrite) {
    ...
    b = b.ucast<float>(0, assign_error_overflow);
    // Multiple cast_scalars operations should make a chained conversion dtype
    EXPECT_EQ(make_strided_dim_type(make_convert_type(ndt::make_dtype<float>(),
                                    make_convert_type<int, float>(assign_error_inexact), assign_error_overflow)),
              b.get_dtype());
