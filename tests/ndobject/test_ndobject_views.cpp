...
TEST(NDArrayViews, OneDimensionalRawMemory) {
    ...
    a = a.view_as_scalar(make_dtype<char>());
    ...
    b = a.at(1 <= irange() < 73).view_as_scalar<uint64_t>();
...
TEST(NDArrayViews, MultiDimensionalRawMemory) {
    ...
    EXPECT_THROW(b = a.view_as_scalar<int16_t>(), runtime_error);

    b = a.view_as_scalar<int32_t>();
...
TEST(NDArrayViews, ExpressionDType) {
    ...
    EXPECT_THROW(b = a_u2.view_as_scalar<int32_t>(), runtime_error);

    b = a_u2.view_as_scalar<int16_t>();
