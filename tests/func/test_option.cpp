...
TEST(Option, IsAvail)
{
  ...
  x.assign_na();
  EXPECT_TRUE(nd::is_missing(x).as<bool>());
  ...
  EXPECT_FALSE(nd::is_missing(x).as<bool>());
...
TEST(Option, IsAvailArray)
{
  ...
  EXPECT_ARRAY_EQ(nd::is_missing(data), expected);
...
TEST(Option, AssignNAArray)
{
  ...
  EXPECT_ARRAY_EQ(nd::is_missing(a), expected);
}
