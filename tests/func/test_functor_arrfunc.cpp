...
TYPED_TEST_P(FunctorArrfunc_FuncRetRes, FuncRetRes)
{
  ...
  a = nd::empty(ndt::make_fixed_dim(3, ndt::type::make<TypeParam>()));

  a.vals() = vals1[0];
  af = nd::functional::apply(
      static_cast<TypeParam (*)(const TypeParam(&)[3])>(&func1));
  res = af(a);
  EXPECT_EQ(ndt::type::make<TypeParam>(), res.get_type());
  ...
  b = nd::empty(ndt::make_fixed_dim(3, ndt::type::make<TypeParam>()));
  ...
  a = nd::empty(ndt::type::make<TypeParam[2][3]>());
...
TEST(FunctorArrfunc, LambdaFunc)
{
  ...
  res = af(a, b);
  EXPECT_EQ(ndt::type::make<float>(), res.get_type());
  ...
  b = 3.25;
  af = nd::functional::apply(
      [](double x, double y, int z) { return x * z + y; });
  af = nd::functional::elwise(af);
  res = af(a, b, 10);
...
TYPED_TEST_P(FunctorArrfunc_MethRetRes, MethRetRes)
{
  ...
  typedef FuncWrapper<
      TypeParam (*)(const TypeParam(&)[3], const TypeParam(&)[3])> FuncWrapper2;
