...
TEST(Reduction, BuiltinSum_Lift1D_NoIdentity)
{
  ...
  EXPECT_ARRAY_EQ(1.5 - 22.0 + 3.75 + 1.125 - 3.375,
...
TEST(Reduction, BuiltinSum_Lift2D_StridedStrided_ReduceReduce_KeepDim)
{
  ...
  EXPECT_ARRAY_EQ(initializer_list<initializer_list<double>>{
                      {1.5 + 2.0 + 7.0 - 2.25 + 7.0 + 2.125}},
                  f(initializer_list<initializer_list<double>>{
                        {1.5, 2.0, 7.0}, {-2.25, 7.0, 2.125}},
                    kwds("keepdims", true)));
...
TEST(Reduction, BuiltinSum_Lift2D_StridedStrided_BroadcastReduce)
{
  ...
  EXPECT_ARRAY_EQ(initializer_list<double>{1.5 - 2.0},
                  f(initializer_list<initializer_list<double>>{{1.5, -2.0}},
                    kwds("axes", nd::array(initializer_list<int>{1}))));
}
...
TEST(Reduction, BuiltinSum_Lift2D_StridedStrided_ReduceBroadcast)
{
  ...
  EXPECT_ARRAY_EQ(
      (initializer_list<double>{1.5 - 2.25, 2.0 + 7.0, 7.0 + 2.125}),
      f(initializer_list<initializer_list<double>>{{1.5, 2.0, 7.0},
                                                   {-2.25, 7.0, 2.125}},
...
TEST(Reduction, BuiltinSum_Lift3D_StridedStridedStrided_ReduceReduceReduce)
{
  ...
  EXPECT_ARRAY_EQ(
      1.5 - 2.375 + 2.0 + 1.25 + 7.0 - 0.5 - 2.25 + 1.0 + 7.0 + 2.125 + 0.25,
...
TEST(Reduction, BuiltinSum_Lift3D_StridedStridedStrided_BroadcastReduceReduce)
{
  ...
  EXPECT_ARRAY_EQ(
      (initializer_list<double>{1.5 - 2.375 + 2.0 + 1.25 + 7.0 - 0.5,
...
TEST(Reduction, Except)
{
  ...
  EXPECT_THROW(
      nd::functional::reduction(nd::functional::apply([]() { return 0; })),
      invalid_argument);

  // Cannot have a child with more than two arguments
  EXPECT_THROW(nd::functional::reduction(nd::functional::apply([](
                   int x, int y, int z) { return x ? y : z; })),
               invalid_argument);
  EXPECT_THROW(nd::functional::reduction(nd::functional::apply([](
                   int x, double y, double z) { return x ? y : z; })),
               invalid_argument);
}
