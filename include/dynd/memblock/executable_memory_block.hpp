 ...
 */
DYND_API memory_block_ptr make_executable_memory_block(intptr_t chunk_size_bytes = 65536);
 ...
 */
DYND_API void set_executable_memory_runtime_function(memory_block_data *self, char *begin, char *end, char *unwind_data);
#else
// ov: work in progress
 ...
 */
DYND_API void allocate_executable_memory(memory_block_data *self, intptr_t size_bytes, intptr_t alignment, char **out_begin, char **out_end);
 ...
 */
DYND_API void resize_executable_memory(memory_block_data *self, intptr_t size_bytes, char **inout_begin, char **inout_end);

DYND_API void executable_memory_block_debug_print(const memory_block_data *memblock, std::ostream& o, const std::string& indent);
