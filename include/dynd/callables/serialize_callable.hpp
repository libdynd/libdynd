...
namespace nd {
  ...
  public:
    serialize_callable()
        : base_callable(ndt::callable_type::make(ndt::type("bytes"), {ndt::type(Arg0ID)}), sizeof(node_type)) {}
    ...
    void instantiate(call_node *&node, char *DYND_UNUSED(data), kernel_builder *ckb,
      ...
      ckb->emplace_back<serialize_kernel<Arg0ID>>(kernreq, reinterpret_cast<node_type *>(node)->data_size);
      node = next(node);
    }
  };
