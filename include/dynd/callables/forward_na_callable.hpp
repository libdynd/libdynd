...
namespace nd {
  ...
  template <>
  class option_comparison_callable<true, true> : public base_callable {
  public:
    option_comparison_callable() : base_callable(ndt::type("(?Scalar, ?Scalar) -> ?bool")) {}

    ndt::type resolve(base_callable *caller, char *DYND_UNUSED(data), call_graph &cg,
      ...
      ndt::type res_value_tp =
          caller->resolve(this, nullptr, cg, ndt::make_type<bool1>(), 2, src_value_tp, nkwd, kwds, tp_vars);
