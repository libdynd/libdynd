...
namespace nd {
  ...
  class not_equal_callable : public default_instantiable_callable<not_equal_kernel<Arg0Type, Arg1Type>> {
  public:
    not_equal_callable()
        : default_instantiable_callable<not_equal_kernel<Arg0Type, Arg1Type>>(ndt::make_type<ndt::callable_type>(
              ndt::make_type<bool1>(), {ndt::make_type<Arg0Type>(), ndt::make_type<Arg1Type>()})) {}
  };

  template <>
  class not_equal_callable<ndt::tuple_type, ndt::tuple_type> : public base_callable {
  public:
    not_equal_callable()
    ...
    ndt::type resolve(base_callable *DYND_UNUSED(caller), char *DYND_UNUSED(data), call_graph &cg,
      ...
      cg.emplace_back([field_count, arrmeta_offsets](kernel_builder &kb, kernel_request_t kernreq,
        ...
        kb.emplace_back<not_equal_kernel<ndt::tuple_type, ndt::tuple_type>>(
        ...
        not_equal_kernel<ndt::tuple_type, ndt::tuple_type> *e =
            kb.get_at<not_equal_kernel<ndt::tuple_type, ndt::tuple_type>>(self_offset);
        ...
        for (size_t i = 0; i != field_count; ++i) {
          ...
          // move the memory.
          e = kb.get_at<not_equal_kernel<ndt::tuple_type, ndt::tuple_type>>(self_offset);
  ...
  template <>
  class not_equal_callable<ndt::struct_type, ndt::struct_type> : public base_callable {
  public:
    not_equal_callable()
        : base_callable(ndt::make_type<ndt::callable_type>(
              ndt::make_type<bool1>(), {ndt::make_type<ndt::struct_type>(), ndt::make_type<ndt::struct_type>()})) {}
    ...
    ndt::type resolve(base_callable *DYND_UNUSED(caller), char *DYND_UNUSED(data), call_graph &cg,
      ...
      cg.emplace_back([field_count, arrmeta_offsets](kernel_builder &kb, kernel_request_t kernreq,
        ...
        kb.emplace_back<not_equal_kernel<ndt::struct_type, ndt::struct_type>>(
            kernreq, field_count, reinterpret_cast<const uintptr_t *>(src_arrmeta[0]),
            reinterpret_cast<const uintptr_t *>(src_arrmeta[1]));
        kb.emplace_back(field_count * sizeof(size_t));

        not_equal_kernel<ndt::struct_type, ndt::struct_type> *e =
            kb.get_at<not_equal_kernel<ndt::struct_type, ndt::struct_type>>(self_offset);
        size_t *field_kernel_offsets;
        for (size_t i = 0; i != field_count; ++i) {
          // Reserve space for the child, and save the offset to this
          // field comparison kernel. Have to re-get
          // the pointer because creating the field comparison kernel may
          // move the memory.
          e = kb.get_at<not_equal_kernel<ndt::struct_type, ndt::struct_type>>(self_offset);
          field_kernel_offsets = reinterpret_cast<size_t *>(e + 1);
          field_kernel_offsets[i] = kb.size() - self_offset;
          const char *field_arrmeta = src_arrmeta[0] + arrmeta_offsets[i];
          const char *child_src_arrmeta[2] = {field_arrmeta, field_arrmeta};
