...
namespace nd {
  ...
  class string_to_int_assign_callable : public base_callable {
    struct node_type : call_node {
      ...
      node_type(base_callable *callee, type_id_t dst_id) : call_node(callee), dst_id(dst_id) {}

      ndt::type string_type() {
        return dst_id == fixed_string_id ? ndt::make_type<ndt::fixed_string_type>(string_size, string_encoding)
                                         : ndt::make_type<ndt::string_type>();
      }
    };
  ...
  public:
    ...
    ndt::type resolve(base_callable *DYND_UNUSED(caller), char *DYND_UNUSED(data), call_graph &cg,
                      ...
                      size_t DYND_UNUSED(nkwd), const array *DYND_UNUSED(kwds),
                      const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars)) {
      node_type *node = cg.emplace_back<node_type>(this, dst_tp.get_id());
      if (node->dst_id == fixed_string_id) {
        node->string_size = dst_tp.extended<ndt::fixed_string_type>()->get_size();
        node->string_encoding = dst_tp.extended<ndt::fixed_string_type>()->get_encoding();
      }
    ...
    void instantiate(call_node *&node, char *DYND_UNUSED(data), kernel_builder *ckb,
                     ...
                     const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars)) {
      ckb->emplace_back<detail::assignment_kernel<IntID, int_kind_id, string_id, string_kind_id, assign_error_default>>(
          kernreq, reinterpret_cast<node_type *>(node)->string_type(), dst_arrmeta);
      node = next(node);
    }
  };
  ...
  public:
    ...
    ndt::type resolve(base_callable *DYND_UNUSED(caller), char *DYND_UNUSED(data), call_graph &cg,
      ...
      assign_error_mode error_mode = kwds[0].is_na() ? assign_error_default : kwds[0].as<assign_error_mode>();
      cg.push_back([dst_encoding, src0_data_size, src0_encoding, error_mode](
          call_node *&node, kernel_builder *ckb, kernel_request_t kernreq, const char *DYND_UNUSED(dst_arrmeta),
          intptr_t DYND_UNUSED(nsrc), const char *const *DYND_UNUSED(src_arrmeta)) {
  ...
  public:
    ...
    ndt::type resolve(base_callable *DYND_UNUSED(caller), char *DYND_UNUSED(data), call_graph &cg,
      ...
      string_encoding_t src0_encoding = src_fs->get_encoding();
      cg.push_back([error_mode, dst_data_size, dst_encoding, src0_encoding](
          call_node *&node, kernel_builder *ckb, kernel_request_t kernreq, const char *DYND_UNUSED(dst_arrmeta),
          intptr_t DYND_UNUSED(nsrc), const char *const *DYND_UNUSED(src_arrmeta)) {
        ckb->emplace_back<detail::assignment_kernel<fixed_string_id, string_kind_id, string_id, string_kind_id,
                                                    assign_error_nocheck>>(
  ...
  public:
    ...
    ndt::type resolve(base_callable *DYND_UNUSED(caller), char *DYND_UNUSED(data), call_graph &cg,
      ...
      cg.push_back([](call_node *&node, kernel_builder *ckb, kernel_request_t kernreq, const char *dst_arrmeta,
        ...
        node->instantiate(node, ckb, kernreq | kernel_request_data_only, nullptr, nsrc, src_arrmeta);
        ...
        node->instantiate(node, ckb, kernreq | kernel_request_data_only, dst_arrmeta, nsrc, nullptr);
        ...
        node->instantiate(node, ckb, kernreq | kernel_request_data_only, dst_arrmeta, 1, src_arrmeta);
      });
  ...
  public:
    ...
    ndt::type resolve(base_callable *DYND_UNUSED(caller), char *DYND_UNUSED(data), call_graph &cg,
      ...
      case float32_id:
      case float64_id:
        cg.push_back([tid, error_mode](call_node *&node, kernel_builder *ckb, kernel_request_t kernreq,
                                       const char *DYND_UNUSED(dst_arrmeta), intptr_t DYND_UNUSED(nsrc),
                                       const char *const *DYND_UNUSED(src_arrmeta)) {
      ...
      case string_id:
        cg.push_back([](call_node *&node, kernel_builder *ckb, kernel_request_t kernreq, const char *dst_arrmeta,
                        intptr_t nsrc, const char *const *src_arrmeta) {
          node->instantiate(node, ckb, kernreq, dst_arrmeta, nsrc, src_arrmeta);
        });
        break;
      default:
        cg.push_back([](call_node *&node, kernel_builder *ckb, kernel_request_t kernreq, const char *dst_arrmeta,
          ...
          node->instantiate(node, ckb, kernreq | kernel_request_data_only, dst_arrmeta, nsrc, src_arrmeta);
          ckb_offset = ckb->size();
  ...
  public:
    ...
    ndt::type resolve(base_callable *DYND_UNUSED(caller), char *DYND_UNUSED(data), call_graph &cg,
      ...
      for (int i = 0; i < field_count; ++i) {
        node->src_arrmeta_offsets[i] = src_arrmeta_offsets[i];
    ...
    void instantiate(call_node *&node, char *DYND_UNUSED(data), kernel_builder *ckb, const ndt::type &dst_tp,
      ...
      for (intptr_t i = 0; i != field_count; ++i) {
        src_fields_arrmeta[i] = src_arrmeta[0] + reinterpret_cast<node_type *>(node)->src_arrmeta_offsets[i];
      }

      const auto &dst_arrmeta_offsets = reinterpret_cast<node_type *>(node)->dst_arrmeta_offsets;
      ...
      for (intptr_t i = 0; i < field_count; ++i) {
        ...
        node->callee->instantiate(node, NULL, ckb, dst_tp, dst_fields_arrmeta[i], 1, src_tp, &src_fields_arrmeta[i],
                                  kernel_request_single, nkwd, kwds, tp_vars);
      }
    }
  };
  ...
  public:
    ...
    ndt::type resolve(base_callable *DYND_UNUSED(caller), char *DYND_UNUSED(data), call_graph &cg,
      ...
      cg.push_back([field_count, src_permutation, src_fields_arrmeta_offsets, dst_arrmeta_offsets](
        ...
        for (intptr_t i = 0; i < field_count; ++i) {
          ...
          field.src_data_offset = src_data_offsets[i];
          node->instantiate(node, ckb, kernel_request_single, dst_fields_arrmeta[i], 1, &src_fields_arrmeta[i]);
        }
      });
