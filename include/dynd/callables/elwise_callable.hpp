...
namespace nd {
  namespace functional {
    ...
    public:
      ...
      ndt::type resolve2(base_callable *parent, call_graph &cg, const ndt::type &dst_tp, size_t nsrc,
        ...
        if (finished) {
          return ndt::make_type<ndt::fixed_dim_type>(
              dst_size, child->resolve(cg, dst_variadic ? child_tp->get_return_type() : dst_element_tp, nsrc,
                                       src_element_tp.data(), nkwd, kwds, tp_vars));
        }

        return ndt::make_type<ndt::fixed_dim_type>(
            dst_size, parent->resolve(cg, dst_element_tp, nsrc, src_element_tp.data(), nkwd, kwds, tp_vars));
      }
      ...
      ndt::type resolve(call_graph &cg, const ndt::type &dst_tp, size_t DYND_UNUSED(nsrc),
        ...
        return ndt::make_type<ndt::base_fixed_dim_type>(dst_tp);
      }
