...
namespace nd {
  ...
  public:
    ...
    ndt::type resolve(base_callable *DYND_UNUSED(caller), char *DYND_UNUSED(data), call_graph &cg,
      ...
      return ndt::make_type<ndt::option_type>(
          Callable->resolve(this, nullptr, cg, dst_tp, 2, child_src_tp, nkwd, kwds, tp_vars));
    }
    ...
    void instantiate(call_node *DYND_UNUSED(node), char *data, kernel_builder *ckb, const ndt::type &dst_tp,
      ...
      is_na->instantiate(nullptr, data, ckb, dst_tp, dst_arrmeta, nsrc, src_tp, src_arrmeta, kernel_request_single,
                         nkwd, kwds, tp_vars);
      ckb_offset = ckb->size();
      ...
      const ndt::type child_src_tp[2] = {src_tp[0].extended<ndt::option_type>()->get_value_type(), src_tp[1]};
      Callable->instantiate(nullptr, data, ckb, dst_tp, dst_arrmeta, nsrc, child_src_tp, src_arrmeta,
      ...
      assign_na->instantiate(nullptr, data, ckb, dst_tp, dst_arrmeta, nsrc, child_src_tp, src_arrmeta,
                             kernel_request_single, nkwd, kwds, tp_vars);
      ckb_offset = ckb->size();
    }
  };
  ...
  public:
    ...
    void instantiate(call_node *DYND_UNUSED(node), char *data, kernel_builder *ckb, const ndt::type &dst_tp,
      ...
      is_na->instantiate(nullptr, data, ckb, dst_tp, dst_arrmeta, nsrc, &src_tp[1], &src_arrmeta[1],
                         kernel_request_single, nkwd, kwds, tp_vars);
      ckb_offset = ckb->size();
      ...
      auto arith = Callable;
      const ndt::type child_src_tp[2] = {src_tp[0], src_tp[1].extended<ndt::option_type>()->get_value_type()};
      arith.get()->instantiate(nullptr, data, ckb, dst_tp, dst_arrmeta, nsrc, child_src_tp, src_arrmeta,
                               kernel_request_single, nkwd, kwds, tp_vars);
      ckb_offset = ckb->size();
      self = ckb->get_at<option_arithmetic_kernel<false, true>>(option_arith_offset);
      self->assign_na_offset = ckb_offset - option_arith_offset;
      assign_na->instantiate(nullptr, data, ckb, src_tp[1], src_arrmeta[1], 0, nullptr, nullptr, kernel_request_single,
                             nkwd, kwds, tp_vars);
      ckb_offset = ckb->size();
    }
  };
  ...
  public:
    ...
    void instantiate(call_node *DYND_UNUSED(node), char *data, kernel_builder *ckb, const ndt::type &dst_tp,
      ...
      self = ckb->get_at<option_arithmetic_kernel<true, true>>(option_arith_offset);
      self->assign_na_offset = ckb_offset - option_arith_offset;
      assign_na->instantiate(nullptr, data, ckb, dst_tp, dst_arrmeta, 0, nullptr, nullptr, kernel_request_single, nkwd,
                             kwds, tp_vars);
      ckb_offset = ckb->size();
    }
  };
