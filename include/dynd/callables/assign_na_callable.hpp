      ...
      case bool_id:
        cg.push_back(
            [](call_node *&DYND_UNUSED(node), kernel_builder * ckb, kernel_request_t kernreq,
               ...
               const char *const *DYND_UNUSED(src_arrmeta)) { ckb->emplace_back<assign_na_kernel<bool_id>>(kernreq); });
        break;
      case int8_id:
        cg.push_back(
            [](call_node *&DYND_UNUSED(node), kernel_builder * ckb, kernel_request_t kernreq,
               const char *DYND_UNUSED(dst_arrmeta), size_t DYND_UNUSED(nsrc),
               const char *const *DYND_UNUSED(src_arrmeta)) { ckb->emplace_back<assign_na_kernel<int8_id>>(kernreq); });
        break;
      case int16_id:
