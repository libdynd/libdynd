...
namespace nd {
  namespace functional {
    namespace detail {
      ...
      template <typename func_type, typename ReturnType, typename ArgSequence, typename KwdSequence, int N>
      class apply_callable_callable : public base_apply_callable<ReturnType, ArgSequence, KwdSequence> {
        func_type m_func;

      public:
        template <typename... T>
        apply_callable_callable(func_type func, T &&... names)
            : base_apply_callable<ReturnType, ArgSequence, KwdSequence>(std::forward<T>(names)...), m_func(func) {}
    ...
    using apply_callable_callable =
        detail::apply_callable_callable<FuncType, typename return_of<FuncType>::type, args_for<FuncType, N>,
                                        as_apply_kwd_sequence<FuncType, N>, N>;
