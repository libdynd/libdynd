...
namespace nd {
  ...
  template <typename OverloaderType>
  struct dispatch_callable<OverloaderType> : base_callable {
    ...
    dispatch_callable(const ndt::type &tp, kernel_targets_t targets, const volatile char *ir, callable_alloc_t alloc,
        ...
        : base_callable(tp, targets, ir, alloc, data_init, resolve_dst_type, instantiate), overloader(overloader)
    {
    }
    ...
    {
      return overloader(ret_tp, narg, arg_tp);
    }
  };
