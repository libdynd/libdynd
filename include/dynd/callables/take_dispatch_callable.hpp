...
namespace nd {
  ...
  public:
    ...
    void instantiate(call_node *&node, char *DYND_UNUSED(data), kernel_builder *ckb,
      ...
      nd::array error_mode = assign_error_default;
      node->callee->instantiate(node, NULL, ckb, ndt::type(), dst_arrmeta + sizeof(ndt::var_dim_type::metadata_type), 1,
                                nullptr, &src0_el_meta, kernel_request_strided, 1, &error_mode, tp_vars);
    }
  };
