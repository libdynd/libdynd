...
namespace nd {
  ...
  struct call_kernel : base_virtual_kernel<call_kernel<CallableType>> {
    static void data_init(const ndt::arrfunc_type *DYND_UNUSED(self_tp),
                          char *DYND_UNUSED(static_data), size_t data_size,
                          char *data, const ndt::type &dst_tp, intptr_t nsrc,
                          const ndt::type *src_tp, const nd::array &kwds,
                          const std::map<dynd::nd::string, ndt::type> &tp_vars)
    {
      CallableType::get()->data_init(
          CallableType::get_type(), CallableType::get()->static_data, data_size,
          data, dst_tp, nsrc, src_tp, kwds, tp_vars);
    }
    ...
    resolve_dst_type(const ndt::arrfunc_type *DYND_UNUSED(self_tp),
                     ...
                     const std::map<dynd::nd::string, ndt::type> &tp_vars)
    {
      CallableType::get()->resolve_dst_type(
          CallableType::get_type(), CallableType::get()->static_data, data_size,
          data, dst_tp, nsrc, src_tp, kwds, tp_vars);
    }
    ...
    instantiate(const arrfunc_type_data *DYND_UNUSED(self),
                ...
                const char *DYND_UNUSED(static_data), size_t data_size,
