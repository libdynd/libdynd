  ...
  namespace detail {
    ...
    */
    ...
    struct assignment_kernel<date_type_id, datetime_kind, string_type_id, string_kind, ErrorMode>
      ...
      {
        assignment_kernel::make(ckb, kernreq, src_tp[0], src_arrmeta[0], date_parse_no_ambig, 70);
      }
    };
    ...
    struct assignment_kernel<string_type_id, string_kind, date_type_id, datetime_kind, ErrorMode>
      ...
      static void instantiate(char *DYND_UNUSED(static_data), char *DYND_UNUSED(data), kernel_builder *ckb,
                              ...
                              const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
      {
        assignment_kernel::make(ckb, kernreq, dst_tp, dst_arrmeta);
      }
    };
    ...
    struct assignment_kernel<datetime_type_id, datetime_kind, string_type_id, string_kind, ErrorMode>
      ...
      static void instantiate(char *DYND_UNUSED(static_data), char *DYND_UNUSED(data), kernel_builder *ckb,
                              ...
                              const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
      {
        assignment_kernel::make(ckb, kernreq, dst_tp, src_tp[0], src_arrmeta[0], date_parse_no_ambig, 70);
      }
    };
    ...
    struct assignment_kernel<string_type_id, string_kind, datetime_type_id, datetime_kind, ErrorMode>
      ...
      static void instantiate(char *DYND_UNUSED(static_data), char *DYND_UNUSED(data), kernel_builder *ckb,
                              ...
                              const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
      {
        assignment_kernel::make(ckb, kernreq, dst_tp, dst_arrmeta, src_tp[0]);
      }
    };
    ...
    struct assignment_kernel<time_type_id, datetime_kind, string_type_id, string_kind, ErrorMode>
      ...
      static void instantiate(char *DYND_UNUSED(static_data), char *DYND_UNUSED(data), kernel_builder *ckb,
                              ...
                              const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
      {
        assignment_kernel::make(ckb, kernreq, src_tp[0], src_arrmeta[0]);
      }
    };
    ...
    struct assignment_kernel<option_type_id, option_kind, string_type_id, string_kind, ErrorMode> {
                              ...
                              intptr_t nkwd, const nd::array *kwds, const std::map<std::string, ndt::type> &tp_vars)
      {
        ...
        switch (tid) {
        case bool_type_id: {
          string_to_option_bool_ck *self = string_to_option_bool_ck::make(ckb, kernreq);
        ...
        case float64_type_id: {
          string_to_option_number_ck *self = string_to_option_number_ck::make(ckb, kernreq);
          self->m_tid = tid;
          self->m_errmode = ErrorMode;
          return;
        }
        ...
        intptr_t root_ckb_offset = ckb_offset;
        string_to_option_tp_ck *self = string_to_option_tp_ck::make(ckb, kernreq);
        ckb_offset = ckb->m_size;
  ...
  namespace detail {
    ...
    struct assignment_kernel<string_type_id, string_kind, Src0TypeID, sint_kind, ErrorMode>
      ...
      static void instantiate(char *DYND_UNUSED(static_data), char *DYND_UNUSED(data), kernel_builder *ckb,
                              ...
                              const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
      {
        assignment_kernel::make(ckb, kernreq, dst_tp, src_tp[0].get_type_id(), dst_arrmeta);
      }
    };
    ...
    struct assignment_virtual_kernel<tuple_type_id, tuple_kind, tuple_type_id, tuple_kind>
                              ...
                              const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
      {
        if (dst_tp.extended() == src_tp[0].extended()) {
          make_tuple_identical_assignment_kernel(ckb, ckb_offset, dst_tp, dst_arrmeta, src_arrmeta[0], kernreq);
        }
        else if (src_tp[0].get_kind() == tuple_kind || src_tp[0].get_kind() == struct_kind) {
          make_tuple_assignment_kernel(ckb, ckb_offset, dst_tp, dst_arrmeta, src_tp[0], src_arrmeta[0], kernreq);
        }
        else if (src_tp[0].is_builtin()) {
          make_broadcast_to_tuple_assignment_kernel(ckb, ckb_offset, dst_tp, dst_arrmeta, src_tp[0], src_arrmeta[0],
                                                    kernreq);
        }
        else {
          std::stringstream ss;
    ...
    struct assignment_virtual_kernel<struct_type_id, struct_kind, struct_type_id, struct_kind>
                              ...
                              const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
      {
        if (dst_tp.extended() == src_tp[0].extended()) {
          ...
          make_struct_assignment_kernel(ckb, ckb_offset, dst_tp, dst_arrmeta, src_tp[0], src_arrmeta[0], kernreq);
          return;
        }
        else if (src_tp[0].is_builtin()) {
          make_broadcast_to_tuple_assignment_kernel(ckb, ckb_offset, dst_tp, dst_arrmeta, src_tp[0], src_arrmeta[0],
    ...
    struct assignment_kernel<string_type_id, string_kind, fixed_string_type_id, string_kind, ErrorMode>
                              ...
                              const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
      {
        assignment_kernel::make(
            ckb, kernreq, dst_tp.extended<ndt::base_string_type>()->get_encoding(),
    ...
    struct assignment_virtual_kernel<view_type_id, expr_kind, Src0TypeID, Src0TypeKind>
      ...
      static void instantiate(char *DYND_UNUSED(static_data), char *DYND_UNUSED(data), kernel_builder *ckb,
                              ...
                              const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
      {
        make_expression_assignment_kernel(ckb, ckb_offset, dst_tp, dst_arrmeta, src_tp[0], src_arrmeta[0], kernreq,
                                          &eval::default_eval_context);
      }
    };
    ...
    struct assignment_kernel<fixed_string_type_id, string_kind, fixed_string_type_id, string_kind, ErrorMode>
                              ...
                              const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
      {
        ...
        assignment_kernel::make(
            ckb, kernreq, get_next_unicode_codepoint_function(src_fs->get_encoding(), ErrorMode),
    ...
    struct adapt_assign_from_kernel : base_kernel<adapt_assign_from_kernel, 1> {
                              ...
                              const nd::array *kwds, const std::map<std::string, ndt::type> &tp_vars)
      {
        ...
        if (storage_tp.is_expression()) {
          ...
          make(ckb, kernreq, storage_tp.get_canonical_type());
          ckb_offset = ckb->m_size;
    ...
    struct assignment_option_kernel : base_kernel<assignment_option_kernel> {
      static void instantiate(char *DYND_UNUSED(static_data), char *DYND_UNUSED(data), kernel_builder *ckb,
                              ...
                              const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
      {
        kernels::make_option_assignment_kernel(ckb, ckb_offset, dst_tp, dst_arrmeta, src_tp[0], src_arrmeta[0],
                                               kernreq);
      }
    };
                              ...
                              const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
      {
        if (dst_tp == src_tp[0]) {
          ...
          if (dst_target_tp == src_tp[0]) {
            make_value_to_pointer_assignment_kernel(ckb, ckb_offset, dst_target_tp, kernreq);
            return;
          }
        }
        make_expression_assignment_kernel(ckb, ckb_offset, dst_tp, dst_arrmeta, src_tp[0], src_arrmeta[0], kernreq,
    ...
    struct assignment_kernel<type_type_id, type_kind, string_type_id, string_kind, ErrorMode>
      ...
      static void instantiate(char *DYND_UNUSED(static_data), char *DYND_UNUSED(data), kernel_builder *ckb,
                              ...
                              const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
      {
        assignment_kernel *e = assignment_kernel::make(ckb, kernreq);
