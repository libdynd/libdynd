...
{ \
    inline static void assign(float16 *dst, const src_type *src) { \
        ...
        *dst = float16(tmp, assign_error_fractional); \
    } \
}; \
template<class src_type> \
...
{ \
    inline static void assign(float16 *dst, const src_type *src) { \
        ...
        *dst = float16(tmp, assign_error_inexact); \
    } \
};
...
{ \
    DYND_CUDA_HOST_DEVICE inline static void assign(dst_type *dst, const float16 *src) { \
...
{ \
    inline static void assign(dst_type *dst, const float16 *src) { \
        float tmp = static_cast<float>(*src); \
