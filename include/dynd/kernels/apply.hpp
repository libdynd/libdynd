...
namespace nd {
  namespace functional {
    ...
    template <typename... A, size_t... I>
    struct apply_args<type_sequence<A...>,
    ...
    struct apply_kwds<type_sequence<K...>,
                      index_sequence<J...>> : apply_kwd<K, J>... {
      apply_kwds(intptr_t DYND_UNUSED(nkwd), const nd::array *kwds)
...
#define APPLY_MEMBER_FUNCTION_CK(...)                                          \
  ...
  struct apply_member_function_ck<                                             \
    ...
    {                                                                          \
      self_type::make(ckb, kernreq, ckb_offset,                                \
                      reinterpret_cast<data_type *>(static_data)->first,       \
                      dynd::detail::make_value_wrapper(                        \
                          reinterpret_cast<data_type *>(static_data)->second), \
                      args_type(src_tp, src_arrmeta, kwds),                    \
                      kwds_type(nkwd, kwds));                                  \
      return ckb_offset;                                                       \
    }                                                                          \
