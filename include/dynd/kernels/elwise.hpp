...
namespace nd {
  namespace functional {
      ...
      */
...

      {
        arrfunc_type_data *child =
            const_cast<arrfunc_type_data *>(self->get_data_as<dynd::nd::arrfunc>()->get());
        const ndt::arrfunc_type *child_tp =
            self->get_data_as<dynd::nd::arrfunc>()->get_type();
        ...
        if (dst_ndim == 0) {
          ...
          if (i == nsrc) {
            ...
            return child->instantiate(child, child_tp, const_cast<char *>(child->static_data), 0,
        ...
        case fixed_dim_type_id:
          if (src_all_strided) {
            return elwise_ck<fixed_dim_type_id, fixed_dim_type_id,
                             N>::instantiate(self, self_tp, const_cast<char *>(self->static_data),
    ...
    struct elwise_ck<fixed_dim_type_id, fixed_dim_type_id, N>
        : base_kernel<elwise_ck<fixed_dim_type_id, fixed_dim_type_id, N>,
          ...
          const std::map<dynd::nd::string, ndt::type> &tp_vars)
      {
        ...
        if (!finished) {
          return nd::functional::elwise_virtual_ck<N>::instantiate(
              self, self_tp, const_cast<char *>(self->static_data), 0, data, ckb, ckb_offset,
        ...
        return child->instantiate(
            child, child_tp, const_cast<char *>(child->static_data), 0, NULL, ckb, ckb_offset,
            child_dst_tp, child_dst_arrmeta, nsrc, child_src_tp,
            child_src_arrmeta, kernreq, ectx, kwds, tp_vars);
      }
    };
    ...
    struct elwise_ck<fixed_dim_type_id, fixed_dim_type_id, 0>
        : base_kernel<elwise_ck<fixed_dim_type_id, fixed_dim_type_id, 0>,
          ...
          const std::map<dynd::nd::string, ndt::type> &tp_vars)
      {
        ...
        // Instantiate the elementwise handler
        return child->instantiate(child, child_tp, const_cast<char *>(child->static_data), 0, NULL,
    ...
    struct elwise_ck<fixed_dim_type_id, var_dim_type_id, N>
        : base_kernel<elwise_ck<fixed_dim_type_id, var_dim_type_id, N>,
    ...
    struct elwise_ck<fixed_dim_type_id, var_dim_type_id, 0>
        : base_kernel<elwise_ck<fixed_dim_type_id, var_dim_type_id, 0>,
    ...
    struct elwise_ck<var_dim_type_id, fixed_dim_type_id, N>
        : base_kernel<elwise_ck<var_dim_type_id, fixed_dim_type_id, N>,
                      kernel_request_host, N> {
      typedef elwise_ck self_type;
    ...
    struct elwise_ck<var_dim_type_id, fixed_dim_type_id, 0>
        : base_kernel<elwise_ck<var_dim_type_id, fixed_dim_type_id, 0>,
                      kernel_request_host, 0> {
      typedef elwise_ck self_type;
