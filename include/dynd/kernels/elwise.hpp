  ...
  namespace functional {
        ...
        if (dst_ndim == 0) {
          ...
          if (i == nsrc) {
            ...
            return child.get()->instantiate(
                child_tp, child.get()->static_data, 0, NULL, ckb, ckb_offset,
                dst_tp, dst_arrmeta, nsrc, src_tp, src_arrmeta, kernreq, ectx,
                kwds, tp_vars);
          } else {
            intptr_t src_ndim =
                src_tp[i].get_ndim() - child_tp->get_pos_type(i).get_ndim();
        ...
        case fixed_dim_type_id:
          if (src_all_strided) {
            return elwise_ck<fixed_dim_type_id, fixed_dim_type_id,
                             N>::instantiate(self_tp, static_data, 0, data, ckb,
                  ...
                  const std::map<dynd::nd::string, ndt::type> &tp_vars)
      {
        ...
        if (!finished) {
          return nd::functional::elwise_virtual_ck<N>::instantiate(
              self_tp, static_data, 0, data, ckb, ckb_offset, child_dst_tp,
    ...
    struct elwise_ck<fixed_dim_type_id, fixed_dim_type_id, 0>
          ...
          const std::map<dynd::nd::string, ndt::type> &tp_vars)
      {
        ...
        // Instantiate the elementwise handler
        return child.get()->instantiate(child_tp, child.get()->static_data, 0,
                                        NULL, ckb, ckb_offset, child_dst_tp,
