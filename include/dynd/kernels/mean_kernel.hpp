  ...
  struct mean_kernel : base_kernel<mean_kernel, 1> {
                                ...
                                const std::map<std::string, ndt::type> &tp_vars)
    {
      ...
      ckb_offset = nd::sum::get().get()->instantiate(
          nd::sum::get().get()->static_data, data_size, reinterpret_cast<data_type *>(data)->sum_data, ckb, ckb_offset,
          dst_tp, dst_arrmeta, nsrc, src_tp, src_arrmeta, kernreq, ectx, nkwd, kwds, tp_vars);
      ...
      ckb_offset = nd::compound_div::get().get()->instantiate(
          nd::compound_div::get().get()->static_data, data_size, reinterpret_cast<data_type *>(data)->compound_div_data,
          ckb, ckb_offset, dst_tp, dst_arrmeta, 1, reinterpret_cast<ndt::type *>(static_data), NULL, kernreq, ectx,
          nkwd, kwds, tp_vars);
