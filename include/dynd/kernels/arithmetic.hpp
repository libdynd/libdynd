...
#undef DYND_DeclBinopKernel
  ...
  struct option_arithmetic_kernel<FuncType, true, false> : base_kernel<option_arithmetic_kernel<FuncType, true, false>,
                                ...
                                const std::map<std::string, ndt::type> &tp_vars)
    {
      ...
      const ndt::type child_src_tp[2] = {src_tp[0].extended<ndt::option_type>()->get_value_type(), src_tp[1]};
      ckb_offset = arith.get()->instantiate(arith.get()->static_data, arith.get()->data_size, data, ckb, ckb_offset,
                                            dst_tp, dst_arrmeta, nsrc, child_src_tp, src_arrmeta, kernel_request_single,
      ...
      ckb_offset = assign_na.get()->instantiate(assign_na.get()->static_data, assign_na.get()->data_size, data, ckb,
                                                ckb_offset, dst_tp, dst_arrmeta, nsrc, child_src_tp, src_arrmeta,
                                                kernel_request_single, ectx, nkwd, kwds, tp_vars);
      return ckb_offset;
    }
  };
  ...
  struct option_arithmetic_kernel<FuncType, false, true> : base_kernel<option_arithmetic_kernel<FuncType, false, true>,
                                ...
                                const std::map<std::string, ndt::type> &tp_vars)
    {
      ...
      auto assign_na = nd::assign_na_decl::get();
      ckb_offset = assign_na.get()->instantiate(assign_na.get()->static_data, assign_na.get()->data_size, data, ckb,
                                                ckb_offset, src_tp[1], src_arrmeta[1], 0, nullptr, nullptr,
                                                kernel_request_single, ectx, nkwd, kwds, tp_vars);
      return ckb_offset;
    }
  };
  ...
  struct option_arithmetic_kernel<FuncType, true, true> : base_kernel<option_arithmetic_kernel<FuncType, true, true>,
                                ...
                                const std::map<std::string, ndt::type> &tp_vars)
    {
      ...
      auto assign_na = nd::assign_na_decl::get();
      ckb_offset = assign_na.get()->instantiate(assign_na.get()->static_data, assign_na.get()->data_size, data, ckb,
                                                ckb_offset, dst_tp, dst_arrmeta, 0, nullptr, nullptr,
                                                kernel_request_single, ectx, nkwd, kwds, tp_vars);
      return ckb_offset;
    }
  };
