...
struct fftw_ck : kernels::expr_ck<fftw_ck<fftw_dst_type, fftw_src_type, sign>,
      ...
      const eval::eval_context *DYND_UNUSED(ectx), const nd::array &kwds)
  {
    ...
    for (intptr_t i = 0; i < rank; ++i) {
      ...
      dims[i].n = shape.is_null() ? src_size_stride[j].dim_size
                                  : shape(j).as<intptr_t>();
    ...
    for (intptr_t i = 0, j = 0, k = 0; i < howmany_rank; ++i, ++j) {
      ...
      howmany_dims[i].n = shape.is_null() ? src_size_stride[j].dim_size
                                          : shape(j).as<intptr_t>();
