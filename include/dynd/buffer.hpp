...
#define __ASSERT_BUFFERED_ARGS(N, TYPES, ASSERT_AUX_BUFFER_ARG_TYPES, ASSERT_THREAD_AUX_BUFFER_ARG_TYPES) \
    ...
    struct assert_buffered_args<R (DYND_PP_JOIN_1((,), TYPES))> { \
        ...
        DYND_STATIC_ASSERT(!DYND_PP_JOIN_MAP_1(VALUE, (||), DYND_PP_POP(ASSERT_THREAD_AUX_BUFFER_ARG_TYPES)), \
            THREAD_AUX_BUFFER_MESSAGE); \
        DYND_STATIC_ASSERT(!(is_thread_aux_buffered::value && \
            (DYND_PP_JOIN_MAP_1(VALUE, (||), DYND_PP_POP(DYND_PP_POP(ASSERT_AUX_BUFFER_ARG_TYPES))))), AUX_BUFFER_MESSAGE); \
        DYND_STATIC_ASSERT(!(!is_thread_aux_buffered::value && \
            (DYND_PP_JOIN_MAP_1(VALUE, (||), DYND_PP_POP(ASSERT_AUX_BUFFER_ARG_TYPES)))), AUX_BUFFER_MESSAGE); \
    };
