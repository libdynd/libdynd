  ...
  struct container_init {
                ...
                metadata + tp.extended<ndt::base_dim_type>()->get_element_arrmeta_offset())
    {
      ...
      case fixed_dim_type_id:
        func = [](const container_init *self, char *data, const ContainerType &values) {
      ...
      case var_dim_type_id:
        func = [](const container_init *self, char *data, const ContainerType &values) {
          for (const value_type &value : values) {
            self->child.single(reinterpret_cast<ndt::var_dim_type::data_type *>(data)->begin, value);
            data += self->stride;
          }
        };
        stride = reinterpret_cast<const ndt::var_dim_type::metadata_type *>(metadata)->stride;
        break;
      default:
    ...
    void single(char *data, const ContainerType &values) const { func(this, data, values); }
  };
