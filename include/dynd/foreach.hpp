...
template<typename R, int N, typename T0, typename T1>
struct foreach_ckernel_instantiator<void (*)(R (&)[N], T0, T1)> {
    typedef foreach_ckernel_instantiator extra_type;
    typedef void (*func_type)(R (&)[N], T0, T1);
    ...
    {
        extra_type *e = reinterpret_cast<extra_type *>(ckp);
        e->func(*reinterpret_cast<R(*)[N]>(dst),
                *reinterpret_cast<const T0 *>(src[0]),
                *reinterpret_cast<const T1 *>(src[1]));
    }
...
template<typename R, int N, typename T0, typename T1>
inline nd::array foreach(const nd::array& a, const nd::array& b, void (*func)(R (&)[N], T0, T1))
{
    // No casting for now
    if (a.get_dtype() != ndt::make_type<T0>()) {
        stringstream ss;
    ...
    ckd.data_types_size = 3;
// ndt::type data_dynd_types[3] = {ndt::make_type<R>(), ndt::make_type<T0>(), ndt::make_type<T1>()};
    ndt::type data_dynd_types[3] = {ndt::make_fixed_dim(1, shape, ndt::make_type<R>(), NULL), ndt::make_type<T0>(), ndt::make_type<T1>()};
// ndt::type data_dynd_types[3] = {ndt::make_strided_dim(ndt::make_type<R>()), ndt::make_type<T0>(), ndt::make_type<T1>()};
    ckd.data_dynd_types = data_dynd_types;
    ckd.data_ptr = reinterpret_cast<void *>(func);
    ckd.instantiate_func = &detail::foreach_ckernel_instantiator<void (*)(R (&)[N], T0, T1)>::instantiate;
    ckd.free_func = NULL;

    // Get the broadcasted shape
    // TODO: This was hastily grabbed from arithmetic_op.cpp, should be encapsulated much better
    size_t ndim = max(a.get_ndim(), b.get_ndim());
// size_t extra_ndim = ndim + 1;
    dimvector result_shape(ndim), tmp_shape(ndim);
    for (size_t j = 0; j != ndim; ++j) {
        result_shape[j] = 1;
    }
