...
public:
    expr_dtype(const ndt::type& value_type, const ndt::type& operand_type,
 ...
 * Makes an expr dtype.
 */
inline ndt::type make_expr_dtype(const ndt::type& value_type,
                const ndt::type& operand_type,
                const expr_kernel_generator *kgen)
{
    return ndt::type(new expr_dtype(value_type, operand_type, kgen), false);
}
