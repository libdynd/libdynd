...
public:
    pointer_dtype(const ndt::type& target_dtype);
...
inline ndt::type make_pointer_dtype(const ndt::type& target_dtype) {
    if (target_dtype.get_type_id() != void_type_id) {
        return ndt::type(new pointer_dtype(target_dtype), false);
    } else {
...
ndt::type make_pointer_dtype() {
    return make_pointer_dtype(ndt::make_dtype<Tnative>());
}
