...
namespace gfunc {
      ...
      static void set(const ndt::type &paramtype, char *arrmeta, char *data, const nd::array &value)
      {
        if (paramtype.get_type_id() == ndarrayarg_type_id) {
          *reinterpret_cast<const array_preamble **>(data) = value.get();
          ...
          typed_data_assign(paramtype, arrmeta, data, value.get_type(), value.metadata(), value.get()->ptr);
        }
      }
    };
    ...
    if (parameter_count != 0) {
      if (m_first_default_parameter <= 0) {
        ...
        for (intptr_t i = 0; i < parameter_count; ++i) {
          ...
          uintptr_t data_offset = fsdt->get_data_offsets(params.metadata())[i];
          typed_data_copy(fsdt->get_field_type(i), params.metadata() + arrmeta_offset, params.get()->ptr + data_offset,
                          m_default_parameters.metadata() + arrmeta_offset,
                          m_default_parameters.get()->ptr + data_offset);
        }
      } else {
        std::stringstream ss;
  ...
  inline nd::array callable::call(const T &p0) const
  {
    ...
    if (parameter_count != 1) {
      if (parameter_count > 1 && m_first_default_parameter <= 1) {
        ...
        for (intptr_t i = 1; i < parameter_count; ++i) {
          ...
          size_t data_offset = fsdt->get_data_offsets(params.metadata())[i];
          typed_data_copy(fsdt->get_field_type(i), params.metadata() + arrmeta_offset, params.get()->ptr + data_offset,
                          m_default_parameters.metadata() + arrmeta_offset,
                          m_default_parameters.get()->ptr + data_offset);
        }
    ...
    detail::callable_argument_setter<T>::set(fsdt->get_field_type(0), params.metadata() + fsdt->get_arrmeta_offset(0),
                                             params.get()->ptr + fsdt->get_data_offsets(params.metadata())[0], p0);
    return call_generic(params);
  }
  ...
  inline nd::array callable::call(const T0 &p0, const T1 &p1) const
  {
    ...
    detail::callable_argument_setter<T0>::set(fsdt->get_field_type(0), params.metadata() + fsdt->get_arrmeta_offset(0),
                                              params.get()->ptr + fsdt->get_data_offsets(params.metadata())[0], p0);
    detail::callable_argument_setter<T1>::set(fsdt->get_field_type(1), params.metadata() + fsdt->get_arrmeta_offset(1),
                                              params.get()->ptr + fsdt->get_data_offsets(params.metadata())[1], p1);
    return call_generic(params);
  }
  ...
  inline nd::array callable::call(const T0 &p0, const T1 &p1, const T2 &p2) const
  {
    ...
    detail::callable_argument_setter<T1>::set(fsdt->get_field_type(1), params.metadata() + fsdt->get_arrmeta_offset(1),
                                              params.get()->ptr + fsdt->get_data_offsets(params.metadata())[1], p1);
    detail::callable_argument_setter<T2>::set(fsdt->get_field_type(2), params.metadata() + fsdt->get_arrmeta_offset(2),
                                              params.get()->ptr + fsdt->get_data_offsets(params.metadata())[2], p2);
    return call_generic(params);
  }
  ...
  inline nd::array callable::call(const T0 &p0, const T1 &p1, const T2 &p2, const T3 &p3) const
  {
    ...
    detail::callable_argument_setter<T2>::set(fsdt->get_field_type(2), params.metadata() + fsdt->get_arrmeta_offset(2),
                                              params.get()->ptr + fsdt->get_data_offsets(params.metadata())[2], p2);
    detail::callable_argument_setter<T3>::set(fsdt->get_field_type(3), params.metadata() + fsdt->get_arrmeta_offset(3),
                                              params.get()->ptr + fsdt->get_data_offsets(params.metadata())[3], p3);
    return call_generic(params);
  }
  ...
  inline nd::array callable::call(const T0 &p0, const T1 &p1, const T2 &p2, const T3 &p3, const T4 &p4) const
  {
    ...
    detail::callable_argument_setter<T3>::set(fsdt->get_field_type(3), params.metadata() + fsdt->get_arrmeta_offset(3),
                                              params.get()->ptr + fsdt->get_data_offsets(params.metadata())[3], p3);
    detail::callable_argument_setter<T4>::set(fsdt->get_field_type(4), params.metadata() + fsdt->get_arrmeta_offset(4),
                                              params.get()->ptr + fsdt->get_data_offsets(params.metadata())[4], p4);
    return call_generic(params);
  }
