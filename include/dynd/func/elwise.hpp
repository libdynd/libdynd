...
#define ELWISE(N) \
    ...
    nd::array elwise DYND_PP_PREPEND(func_type func, DYND_PP_APPEND(bool copy = true, DYND_PP_META_NAME_RANGE(const nd::array &a, N))) { \
        nd::arrfunc af = make_functor_arrfunc(func, copy); \
...
#define ELWISE(N) \
    ...
    nd::array elwise DYND_PP_PREPEND(obj_type obj, DYND_PP_PREPEND(mem_func_type mem_func, DYND_PP_APPEND(bool copy = true, DYND_PP_META_NAME_RANGE(const nd::array &a, N)))) { \
        nd::arrfunc af = make_functor_arrfunc(obj, mem_func, copy); \
        nd::arrfunc laf = lift_arrfunc(af); \
\
        nd::array args[N] = {DYND_PP_JOIN((,), DYND_PP_META_NAME_RANGE(a, N))}; \
        return laf.call(N, args, &eval::default_eval_context); \
