...
#define MAKE(NSRC, NARG) \
    ...
    static void make(arrfunc_type_data *out_af) { \
        typedef R (funcproto_type) DYND_PP_META_NAME_RANGE(A, NARG); \
\
        out_af->func_proto = ndt::make_funcproto<funcproto_type>(DYND_PP_SUB(NARG, NSRC)); \
        out_af->instantiate = &functor_ck<func_type, funcproto_type, DYND_PP_SUB(NARG, NSRC), true>::instantiate; \
    }
