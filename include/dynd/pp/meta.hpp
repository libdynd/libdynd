//
// Copyright (C) 2011-14 Irwin Zaid, DyND Developers
// BSD 2-Clause License, see LICENSE.txt
//

#ifndef _DYND__PP_META_HPP_
#define _DYND__PP_META_HPP_

#define DYND_PP_META_PARENTHESIZE(A) (A)

#define DYND_PP_META_DECL(TYPE, VAR) TYPE VAR
#define DYND_PP_META_ASGN(LHS, RHS) LHS = RHS
#define DYND_PP_META_DECL_ASGN(TYPE, VAR, VALUE) DYND_PP_META_ASGN(DYND_PP_META_DECL(TYPE, VAR), VALUE)

#define DYND_PP_META_NE(LHS, RHS) LHS != RHS
#define DYND_PP_META_GT(A, B) A > B

#define DYND_PP_META_ADD(A, B) A + B
#define DYND_PP_META_SUB(A, B) A - B
#define DYND_PP_META_MUL(A, B) A * B
#define DYND_PP_META_DIV(A, B) A / B
#define DYND_PP_META_MOD(A, B) A % B

#define DYND_PP_META_ADD_EQ(A, B) A += B 
#define DYND_PP_META_SUB_EQ(A, B) A -= B 

#define DYND_PP_META_NOT(A) !A
#define DYND_PP_META_AND(A, B) A && B
#define DYND_PP_META_OR(A, B) A || B

#define DYND_PP_META_PARENTHESIZED_AND(A, B) DYND_PP_META_PARENTHESIZE(DYND_PP_META_AND(A, B))

#define DYND_PP_META_IF(CONDITION, A) if (CONDITION) {A}
#define DYND_PP_META_IF_ELSE(CONDITION, A, B) if (CONDITION) {A} else {B}

#define DYND_PP_META_AT(VAR, INDEX) VAR[INDEX]
#define DYND_PP_META_DEREFERENCE(VAR) *VAR
#define DYND_PP_META_ADDRESS(VAR) &VAR

#define DYND_PP_META_CALL(FUNC, ...) FUNC(__VA_ARGS__)

#define DYND_PP_META_DOT(LHS, RHS) LHS.RHS
#define DYND_PP_META_DOT_CALL(OBJ, FUNC, ...) DYND_PP_META_DOT(OBJ, DYND_PP_META_CALL(FUNC, __VA_ARGS__))

#define DYND_PP_META_SCOPE(LHS, RHS) LHS::RHS
#define DYND_PP_META_SCOPE_CALL(OBJ, FUNC, ...) DYND_PP_META_SCOPE(OBJ, DYND_PP_META_CALL(FUNC, __VA_ARGS__))

#define DYND_PP_META_TYPENAME(TYPE) typename TYPE

#define DYND_PP_META_TEMPLATE(TEMP, ARGS) TEMP< ARGS >

#define DYND_PP_META_STATIC_CAST(TYPE, VAR) static_cast<TYPE>(VAR)
#define DYND_PP_META_REINTERPRET_CAST(TYPE, VAR) reinterpret_cast<TYPE>(VAR)
#define DYND_PP_META_CONST_CAST(TYPE, VAR) const_cast<TYPE>(VAR)
#define DYND_PP_META_DYNAMIC_CAST(TYPE, VAR) dynamic_cast<TYPE>(VAR)

#define DYND_PP_META_NAME_RANGE(NAME, ...) DYND_PP_OUTER(DYND_PP_PASTE, (NAME), DYND_PP_RANGE(__VA_ARGS__))
#define DYND_PP_META_AT_RANGE(NAME, ...) DYND_PP_OUTER(DYND_PP_META_AT, (NAME), DYND_PP_RANGE(__VA_ARGS__))

#define DYND_PP_META_MAKE_PTR(TYPE) TYPE *
#define DYND_PP_META_MAKE_CONST_PTR(TYPE) const DYND_PP_META_MAKE_PTR(TYPE)

#define DYND_PP_META_TYPEDEF(TYPE, NAME) typedef TYPE NAME
#define DYND_PP_META_TYPEDEF_TYPENAME(TYPE, NAME) typedef typename TYPE NAME

#define DYND_PP_META_SEMICOLON(...) __VA_ARGS__;

#define DYND_PP_META_SEMI(A, B) A; B;

#define DYND_PP_META_ADJACENT(X, Y) DYND_PP_ID(X)DYND_PP_ID(Y)

#define DYND_PP_META_STATEMENTS(A) (DYND_PP_JOIN_1((;), A);)

#define DYND_PP_META_FLAT_CALL(FUNC, ARGS) FUNC ARGS

#define DYND_PP_META_TEMPLATE_SCOPE(A, B, C) DYND_PP_META_SCOPE(DYND_PP_META_TEMPLATE(A, B), C)
#define DYND_PP_META_TYPENAME_TEMPLATE_SCOPE(A, B, C) DYND_PP_META_TYPENAME(DYND_PP_META_TEMPLATE_SCOPE(A, B, C))

#define DYND_PP_META_STATIC_ASSERT(CONDITION, MESSAGE) static_assert(CONDITION, MESSAGE)

#endif
