...
static void linspace_specialization(dynd::complex<float> start, dynd::complex<float> stop, intptr_t count, nd::array& result)
{
    intptr_t stride = result.get_strides()[0];
    char *dst = result.get_readwrite_originptr();
    for (intptr_t i = 0; i < count; ++i, dst += stride) {
        dynd::complex<double> val = (double(count - i - 1) * dynd::complex<double>(start) +
                        double(i) * dynd::complex<double>(stop)) / double(count - 1);
        *reinterpret_cast<dynd::complex<float> *>(dst) = dynd::complex<float>(val);
    }
}

static void linspace_specialization(dynd::complex<double> start, dynd::complex<double> stop, intptr_t count, nd::array& result)
{
    intptr_t stride = result.get_strides()[0];
    char *dst = result.get_readwrite_originptr();
    for (intptr_t i = 0; i < count; ++i, dst += stride) {
