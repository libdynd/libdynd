  ...
  if (value < 2) {
    if (tp.get_type_id() == bool_type_id) {
      *out_data = value;
      ...
      typed_data_assign(tp, arrmeta, out_data, ndt::make_type<dynd_bool>(),
                        NULL, &value);
    }
    rbegin = begin;
  }
    ...
    if (!tp.is_expression()) {
      *out_data = value;
      ...
      typed_data_assign(tp, arrmeta, out_data, ndt::make_option<dynd_bool>(),
                        NULL, &value);
    }
    rbegin = begin;
  }
                              ...
                              const eval::eval_context *ectx)
{
  ...
  if (option && parse::parse_token_no_ws(begin, end, "null")) {
    ndt::make_option(tp).extended<option_type>()->assign_na(arrmeta, out_data,
                                                            ectx);
  }
  else if (parse::parse_json_number_no_ws(begin, end, nbegin, nend)) {
                              ...
                              const char *end, const eval::eval_context *ectx)
{
  ...
  if (tp.is_scalar()) {
    if (parse_token(begin, end, "null")) {
      ...
      const ndt::type &value_tp = tp.extended<option_type>()->get_value_type();
      const char *strbegin, *strend;
      bool escaped;
      if (parse::parse_doublequote_string_no_ws(begin, end, strbegin, strend,
                                                escaped)) {
