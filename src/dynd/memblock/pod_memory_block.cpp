...
struct pod_memory_block {
  ...
  pod_memory_block(intptr_t data_alignment, intptr_t initial_capacity_bytes)
      : m_mbd(1, pod_memory_block_type), data_alignment(data_alignment), m_total_allocated_capacity(0),
        m_memory_handles()
  {
...
{
  pod_memory_block *pmb = new pod_memory_block(tp.get_data_alignment(), initial_capacity_bytes);
...
namespace detail {
  ...
  memory_block_pod_allocator_api pod_memory_block_allocator_api = {&allocate, &resize, &finalize, &reset};
}
