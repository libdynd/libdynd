...
struct executable_memory_block {
  ...
  void append_memory()
  {
    ...
    m_memory_begin = reinterpret_cast<char *>(
        VirtualAllocEx(hProcess, NULL, m_chunk_size_bytes, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE));
  ...
  executable_memory_block(intptr_t chunk_size_bytes)
      : m_mbd(1, executable_memory_block_type), m_total_allocated_capacity(0), m_chunk_size_bytes(chunk_size_bytes),
        m_memory_handles()
  {
    append_memory();
  }
...
void dynd::allocate_executable_memory(memory_block_data *self, intptr_t size_bytes, intptr_t alignment,
                                      char **out_begin, char **out_end)
{
  ...
  char *begin =
      reinterpret_cast<char *>((reinterpret_cast<uintptr_t>(emb->m_memory_current) + alignment - 1) & ~(alignment - 1));
...
void dynd::executable_memory_block_debug_print(const memory_block_data *memblock, std::ostream &o,
                                               const std::string &indent)
{
  ...
  for (size_t i = 0, i_end = emb->m_memory_handles.size(); i != i_end; ++i) {
    ...
    for (size_t j = 0, j_end = vac.m_functions.size(); j != j_end; ++j) {
      ...
      o << ", " << (void *)(uintptr_t)rf.EndAddress << ", " << (void *)(uintptr_t)rf.UnwindData << "}\n";
    }
#endif
  }
}
