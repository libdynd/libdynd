...
intrusive_ptr<memory_block_data> make_executable_memory_block(intptr_t chunk_size_bytes)
{
  executable_memory_block *pmb = new executable_memory_block(chunk_size_bytes);
  return intrusive_ptr<memory_block_data>(pmb, false);
}
                                ...
                                )
{
  ...
  std::cout << "allocating " << size_bytes << " of executable memory with alignment " << alignment << std::endl;
  ...
  emb->m_pivot = end;
#ifndef NDEBUG
  assert(ptr_in_range(begin, emb->m_allocated_chunks.back(),
                      ptr_offset(emb->m_allocated_chunks.back(), emb->m_chunk_size)));
  assert(
      ptr_in_range(end, emb->m_allocated_chunks.back(), ptr_offset(emb->m_allocated_chunks.back(), emb->m_chunk_size)));
#endif
...
void resize_executable_memory(memory_block_data *self, intptr_t new_size, char **inout_begin, char **inout_end)
{
  executable_memory_block *emb = static_cast<executable_memory_block *>(self);
  void *current_chunk = emb->m_allocated_chunks.back();
...
void executable_memory_block_debug_print(const memory_block_data *memblock, std::ostream &os, const std::string &indent)
{
  ...
  size_t allocated = emb->m_allocated_chunks.size() * (chunk_size - 1) + current_chunk_used_bytes;
