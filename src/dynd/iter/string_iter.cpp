...
static dim_iter_vtable transcode_fixed_encoding_string_iter_vt = {
    transcode_string_iter_destructor,
    transcode_string_iter_next,
    transcode_string_iter_seek_fixed_encoding
};

static dim_iter_vtable transcode_var_encoding_string_iter_vt = {
    transcode_string_iter_destructor,
    transcode_string_iter_next,
    transcode_string_iter_seek_var_encoding
};

void iter::make_string_iter(
    ...
    const eval::eval_context *ectx)
{
    ...
    if (datasize == 0) {
        ...
        make_strided_dim_iter(out_di, ctp,
            NULL, data_begin, datasize, ctp.get_data_size(), ref);
        return;
    } else {
        ...
        out_di->custom[5] = reinterpret_cast<uintptr_t>(
            get_next_unicode_codepoint_function(data_encoding, ectx->errmode));
        out_di->custom[6] = reinterpret_cast<uintptr_t>(
            get_append_unicode_codepoint_function(iter_encoding, ectx->errmode));
        if (ref.get() != NULL) {
