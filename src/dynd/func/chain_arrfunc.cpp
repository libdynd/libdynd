    ...
    const eval::eval_context *ectx)
{
  if (first_tp->get_npos() == 1) {
    ...
    ckb_offset = first->instantiate(
        first, first_tp, NULL, ckb, ckb_offset, buf_tp, self->m_buf_arrmeta.get(),
        first_tp->get_npos(), src_tp, src_arrmeta, kernreq, ectx, nd::array(), std::map<nd::string, ndt::type>());
    ...
    ckb_offset = second->instantiate(second, second_tp, NULL, ckb, ckb_offset, dst_tp,
                                     dst_arrmeta, first_tp->get_npos(), &buf_tp, &buf_arrmeta,
                                     ...
                                     const ndt::type &buf_tp)
{
  ...
  nd::array af = nd::empty(ndt::make_arrfunc(
      first.get_type()->get_pos_tuple(), second.get_type()->get_return_type()));
