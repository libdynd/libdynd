...
static intptr_t instantiate_neighborhood(
    ...
    kernel_request_t kernreq, const nd::array &args, const nd::array &kwds, const eval::eval_context *ectx)
{
    ...
    ckb_offset = nh_op.get()->instantiate(nh_op.get(), ckb, ckb_offset,
        ...
        kernel_request_single, args, pack(kwds, "start_stop", reinterpret_cast<intptr_t>(nh->start_stop)), ectx);
    return ckb_offset;
}
