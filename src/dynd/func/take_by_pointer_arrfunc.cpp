...
static intptr_t instantiate_take_by_pointer(const arrfunc_type_data *DYND_UNUSED(af_self),
                                  ...
                                  kernel_request_t kernreq, const nd::array &DYND_UNUSED(args),
                                  const nd::array &DYND_UNUSED(kwds), const eval::eval_context *ectx)
{
    ...
    if (!dst_tp.get_as_strided(dst_arrmeta, 1, &dst_size_stride,
    ...
    take_by_pointer_outer_ck::create(ckb, kernreq, ckb_offset,
        dst_size_stride[0].dim_size, dst_size_stride[0].stride, src_size_stride[1][0].stride);

    for (intptr_t i = 0; i < ndim; ++i) {
        take_by_pointer_ck::create(ckb, kernel_request_single, ckb_offset,
            src_size_stride[0][i].dim_size, src_size_stride[0][i].stride, src_size_stride[1][1].stride);
    }
