...
uint16_t dynd::float_to_halfbits(float value)
{
  union {
    float f;
    uint32_t fbits;
...
uint16_t dynd::double_to_halfbits(double value)
{
  union {
    double d;
    uint64_t dbits;
...
double dynd::halfbits_to_double(uint16_t h)
{
  ...
  case 0x7c00u: // inf or NaN
    // All-ones exponent and a copy of the significand
    conv.dbits =
        d_sgn + 0x7ff0000000000000ULL + (((uint64_t)(h & 0x03ffu)) << 42);
    return conv.d;
  default: // normalized
    // Just need to adjust the exponent and shift
...
dynd::float16::float16(int128 value)
{
  m_bits = double_to_halfbits((double)value);
}

dynd::float16::float16(uint128 value)
{
  m_bits = double_to_halfbits((double)value);
}
