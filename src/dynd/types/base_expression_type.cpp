...
void base_expression_type::arrmeta_default_construct(char *arrmeta, intptr_t ndim, const intptr_t* shape) const
{
...
void base_expression_type::arrmeta_copy_construct(char *dst_arrmeta, const char *src_arrmeta, memory_block_data *embedded_reference) const
{
    const ndt::type& dt = get_operand_type();
    if (!dt.is_builtin()) {
...
void base_expression_type::arrmeta_destruct(char *arrmeta) const
{
    const ndt::type& dt = get_operand_type();
    if (!dt.is_builtin()) {
...
void base_expression_type::arrmeta_debug_print(const char *arrmeta, std::ostream& o, const std::string& indent) const
{
    const ndt::type& dt = get_operand_type();
    if (!dt.is_builtin()) {
