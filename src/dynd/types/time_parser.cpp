...
bool dynd::parse_timezone(const char *&rbegin, const char *end, const char *&out_tz_begin, const char *&out_tz_end)
{
  ...
  if (begin[0] == '+' || begin[0] == '-') {
    ...
    ++begin;
    if (end - begin < 2 || begin[0] < '0' || begin[0] > '9' || begin[1] < '0' || begin[1] > '9') {
      return false;
    }
    begin += 2;
    if (begin == end || !(begin[0] == ':' || (begin[0] >= '0' && begin[0] <= '9'))) {
      // +hh or -hh
      rbegin = begin;
...
static bool parse_flex_time(const char *&begin, const char *end, time_hmst &out_hmst)
{
  ...
  // :MM
  if (!parse_token_no_ws(begin, end, '.') && !parse_token_no_ws(begin, end, ':')) {
...
bool dynd::parse_time_ampm(const char *&begin, const char *end, int &inout_hour)
{
  saved_begin_state sbs(begin);

  skip_whitespace(begin, end);

  if (parse_token(begin, end, "AM") || parse_token(begin, end, "am") || parse_token(begin, end, "A.M.") ||
      parse_token(begin, end, "a.m.") || parse_token(begin, end, "a")) {
  ...
  else if (parse_token(begin, end, "PM") || parse_token(begin, end, "pm") || parse_token(begin, end, "P.M.") ||
           parse_token(begin, end, "p.m.") || parse_token(begin, end, "p")) {
    // The hour should be between 1 and 12
    if (inout_hour < 1 || inout_hour > 12) {
      // Invalidate the hour for later checks to see
      inout_hour = -1;
...
bool dynd::parse_time(const char *&begin, const char *end, time_hmst &out_hmst, const char *&out_tz_begin,
                      const char *&out_tz_end)
{
  if (dynd::parse_time_no_tz(begin, end, out_hmst)) {
...
bool dynd::parse_time_no_tz(const char *&begin, const char *end, time_hmst &out_hmst)
{
...
bool dynd::string_to_time(const char *begin, const char *end, time_hmst &out_hmst, const char *&out_tz_begin,
                          const char *&out_tz_end)
{
  time_hmst hmst;
  skip_whitespace(begin, end);
