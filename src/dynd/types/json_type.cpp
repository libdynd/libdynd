...
ndt::json_type::json_type()
    : base_string_type(json_type_id, sizeof(json_type_data), sizeof(const char *),
...
void ndt::json_type::get_string_range(const char **out_begin, const char **out_end, const char *DYND_UNUSED(arrmeta),
...
void ndt::json_type::set_from_utf8_string(const char *arrmeta, char *dst, const char *utf8_begin, const char *utf8_end,
                                          const eval::eval_context *ectx) const
{
  ...
  const json_type_arrmeta *data_md = reinterpret_cast<const json_type_arrmeta *>(arrmeta);
...
void ndt::json_type::print_data(std::ostream &o, const char *DYND_UNUSED(arrmeta), const char *data) const
{
  uint32_t cp;
...
{
  const json_type_arrmeta *md = reinterpret_cast<const json_type_arrmeta *>(arrmeta);
...
bool ndt::json_type::is_lossless_assignment(const type &dst_tp, const type &src_tp) const
{
...
void ndt::json_type::arrmeta_default_construct(char *arrmeta, bool blockref_alloc) const
{
...
void ndt::json_type::arrmeta_copy_construct(char *dst_arrmeta, const char *src_arrmeta,
                                            memory_block_data *embedded_reference) const
{
  // Copy the blockref, switching it to the embedded_reference if necessary
  const json_type_arrmeta *src_md = reinterpret_cast<const json_type_arrmeta *>(src_arrmeta);
  json_type_arrmeta *dst_md = reinterpret_cast<json_type_arrmeta *>(dst_arrmeta);
...
void ndt::json_type::arrmeta_debug_print(const char *arrmeta, std::ostream &o, const std::string &indent) const
{
  const json_type_arrmeta *md = reinterpret_cast<const json_type_arrmeta *>(arrmeta);
...
struct string_to_json_ck : nd::base_kernel<string_to_json_ck, 1> {
  ...
  {
    const json_type_arrmeta *md = reinterpret_cast<const json_type_arrmeta *>(m_dst_arrmeta);
...
intptr_t ndt::json_type::make_assignment_kernel(void *ckb, intptr_t ckb_offset, const type &dst_tp,
                                                ...
                                                kernel_request_t kernreq, const eval::eval_context *ectx) const
{
  if (this == dst_tp.extended()) {
    ...
    case json_type_id: {
      ...
      return make_blockref_string_assignment_kernel(ckb, ckb_offset, dst_arrmeta, string_encoding_utf_8, src_arrmeta,
                                                    string_encoding_utf_8, kernreq, ectx);
    }
    case string_type_id:
    case fixed_string_type_id: {
      string_to_json_ck *stoj = string_to_json_ck::make(ckb, kernreq, ckb_offset);
      ...
      if (src_tp.get_type_id() == string_type_id) {
        ...
        return make_fixed_string_to_blockref_string_assignment_kernel(
            ckb, ckb_offset, dst_arrmeta, string_encoding_utf_8, src_tp.get_data_size(),
            src_tp.extended<base_string_type>()->get_encoding(), kernel_request_single, ectx);
      }
    }
    default: {
    ...
    if (dst_tp.is_builtin()) {
      ...
      return make_blockref_string_assignment_kernel(ckb, ckb_offset, dst_arrmeta,
...
void ndt::json_type::make_string_iter(dim_iter *out_di, string_encoding_t encoding, const char *arrmeta,
