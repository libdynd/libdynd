...
ndt::unary_expr_type::unary_expr_type(const type &value_type,
                                      const type &operand_type,
                                      const expr_kernel_generator *kgen)
    : base_expr_type(
          unary_expr_type_id, expr_kind, operand_type.get_data_size(),
...
void ndt::unary_expr_type::print_data(std::ostream &DYND_UNUSED(o),
...
ndt::type ndt::unary_expr_type::apply_linear_index(
...
intptr_t ndt::unary_expr_type::apply_linear_index(
...
bool ndt::unary_expr_type::is_lossless_assignment(
...
size_t ndt::unary_expr_type::make_operand_to_value_assignment_kernel(
    ...
    const eval::eval_context *ectx) const
{
  ...
  return m_kgen->make_expr_kernel(ckb, ckb_offset, m_value_type, dst_arrmeta, 1,
                                  &m_operand_type.value_type(), &src_arrmeta,
                                  kernreq, ectx);
}

size_t ndt::unary_expr_type::make_value_to_operand_assignment_kernel(
...
ndt::type ndt::unary_expr_type::with_replaced_storage_type(
    const type &DYND_UNUSED(replacement_type)) const
{
  throw runtime_error(
      "TODO: implement unary_expr_type::with_replaced_storage_type");
}

void ndt::unary_expr_type::get_dynamic_array_properties(
...
void ndt::unary_expr_type::get_dynamic_array_functions(
