...
cfixed_dim_type::cfixed_dim_type(size_t dimension_size,
    ...
    : base_dim_type(cfixed_dim_type_id, element_tp, 0,
                    element_tp.get_data_alignment(), 0, type_flag_none, true),
      m_stride(stride), m_dim_size(dimension_size)
{
  ...
  if (dimension_size <= 1 && stride != 0) {
    ...
    ss << " and stride " << stride
       << ", as the stride must be zero when the dimension size is 1";
    throw dynd::type_error(ss.str());
  }
  if (dimension_size > 1 && stride == 0) {
    stringstream ss;
    ss << "Cannot create dynd cfixed_dim type with size " << dimension_size;
    ss << " and stride 0, as the stride must be non-zero when the dimension "
          "size is > 1";
    throw dynd::type_error(ss.str());
  }
  m_members.data_size = m_stride * (m_dim_size - 1) + child_element_size;
...
void cfixed_dim_type::print_type(std::ostream &o) const
{
  o << "cfixed[";
  o << m_dim_size;
  if ((size_t)m_stride != m_element_tp.get_data_size() && m_dim_size != 1) {
    o << ", stride=" << m_stride;
  }
  o << "] * " << m_element_tp;
}
...
ndt::type cfixed_dim_type::get_canonical_type() const
{
  ...
  if (canonical_element_dt.get_data_size() != 0) {
    return ndt::type(new cfixed_dim_type(m_dim_size, canonical_element_dt),
                     false);
  } else {
    return ndt::type(new fixed_dim_type(m_dim_size, canonical_element_dt),
                     false);
  }
}
        ...
        if (leading_dimension) {
          ...
          *inout_data += offset;
          offset = m_element_tp.extended()->apply_linear_index(
              ...
              arrmeta + sizeof(cfixed_dim_type_arrmeta), result_tp, out_arrmeta,
...
void cfixed_dim_type::arrmeta_default_construct(char *arrmeta,
...
size_t cfixed_dim_type::get_iterdata_size(intptr_t ndim) const
{
  if (ndim == 0) {
    ...
    return m_element_tp.get_iterdata_size(ndim - 1) +
           sizeof(cfixed_dim_type_iterdata);
  }
}
...
{
  cfixed_dim_type_iterdata *id =
      reinterpret_cast<cfixed_dim_type_iterdata *>(iterdata);
  if (level == 0) {
    id->data += id->stride;
    return id->data;
  } else {
...
size_t cfixed_dim_type::iterdata_destruct(iterdata_common *iterdata,
                                          intptr_t ndim) const
{
  size_t inner_size = 0;
  if (ndim > 1) {
                                      ...
                                      void *callback_data) const
{
  ...
  for (intptr_t i = 0, i_end = m_dim_size; i < i_end; ++i, data += stride) {
    callback(m_element_tp, arrmeta + sizeof(cfixed_dim_type_arrmeta), data,
             callback_data);
  }
}
    ...
    size_t *out_count) const
{
  ...
  *out_count = sizeof(cfixed_dim_type_properties) /
               sizeof(cfixed_dim_type_properties[0]);
}
