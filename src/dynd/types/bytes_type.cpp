...
bytes_type::bytes_type(size_t alignment)
    : base_bytes_type(bytes_type_id, bytes_kind, sizeof(bytes_type_data),
                    ...
                    sizeof(bytes_type_arrmeta)), m_alignment(alignment)
{
...
void bytes_type::set_bytes_data(const char *arrmeta, char *data,
...
bool bytes_type::is_unique_data_owner(const char *arrmeta) const
{
    const bytes_type_arrmeta *md = reinterpret_cast<const bytes_type_arrmeta *>(arrmeta);
    if (md->blockref != NULL &&
...
intptr_t bytes_type::make_assignment_kernel(
...
void bytes_type::arrmeta_reset_buffers(char *DYND_UNUSED(arrmeta)) const
{
    throw runtime_error("TODO implement bytes_type::arrmeta_reset_buffers");
}

void bytes_type::arrmeta_finalize_buffers(char *arrmeta) const
{
    bytes_type_arrmeta *md = reinterpret_cast<bytes_type_arrmeta *>(arrmeta);
    if (md->blockref != NULL) {
...
void bytes_type::arrmeta_destruct(char *arrmeta) const
{
    bytes_type_arrmeta *md = reinterpret_cast<bytes_type_arrmeta *>(arrmeta);
    if (md->blockref) {
...
void bytes_type::arrmeta_debug_print(const char *arrmeta, std::ostream& o, const std::string& indent) const
{
    const bytes_type_arrmeta *md = reinterpret_cast<const bytes_type_arrmeta *>(arrmeta);
