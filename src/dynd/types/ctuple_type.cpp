      ...
      m_data_offsets(nd::empty(m_field_count, ndt::make_type<uintptr_t>()))
{
  uintptr_t *data_offsets =
      reinterpret_cast<uintptr_t *>(m_data_offsets.get_readwrite_originptr());
...
bool ctuple_type::is_lossless_assignment(const ndt::type &dst_tp,
...
void ctuple_type::arrmeta_debug_print(const char *arrmeta, std::ostream &o,
...
{
  return tp.extended<ctuple_type>()->get_field_types();
}

static nd::array property_get_data_offsets(const ndt::type &tp)
{
  return tp.extended<ctuple_type>()->get_data_offsets();
}

static nd::array property_get_arrmeta_offsets(const ndt::type &tp)
{
  return tp.extended<ctuple_type>()->get_arrmeta_offsets();
}

void ctuple_type::get_dynamic_type_properties(
