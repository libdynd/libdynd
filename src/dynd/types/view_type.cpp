...
ndt::view_type::view_type(const type &value_type, const type &operand_type)
    : base_expr_type(
          view_type_id, expr_kind, operand_type.get_data_size(),
      ...
      m_value_type(value_type), m_operand_type(operand_type)
{
  if (value_type.get_data_size() != operand_type.value_type().get_data_size()) {
    std::stringstream ss;
    ss << "view_type: Cannot view " << operand_type.value_type() << " as "
       << value_type << " because they have different sizes";
...
ndt::view_type::~view_type() {}

void ndt::view_type::print_data(std::ostream &o, const char *arrmeta,
                                const char *data) const
{
  ...
  if (m_operand_type.get_type_id() == fixed_bytes_type_id) {
    ...
    default: {
      vector<char> storage(m_value_type.get_data_size() +
                           m_value_type.get_data_alignment());
      char *buffer = &storage[0];
      // Make the storage aligned as needed
      buffer = (char *)(((uintptr_t)buffer +
                         (uintptr_t)m_value_type.get_data_alignment() - 1) &
                        (m_value_type.get_data_alignment() - 1));
      memcpy(buffer, data, m_value_type.get_data_size());
      m_value_type.print_data(o, arrmeta,
  ...
  throw runtime_error(
      "internal error: view_type::print_data isn't supposed to be called");
}

void ndt::view_type::print_type(std::ostream &o) const
{
  // Special case printing of alignment to make it more human-readable
  if (m_value_type.get_data_alignment() != 1 &&
      m_operand_type.get_type_id() == fixed_bytes_type_id &&
...
void ndt::view_type::get_shape(intptr_t ndim, intptr_t i, intptr_t *out_shape,
...
bool ndt::view_type::is_lossless_assignment(const type &dst_tp,
...
bool ndt::view_type::operator==(const base_type &rhs) const
{
  if (this == &rhs) {
    return true;
...
ndt::type
ndt::view_type::with_replaced_storage_type(const type &replacement_type) const
{
  if (m_operand_type.get_kind() == expr_kind) {
    return type(
        new view_type(m_value_type,
    ...
    if (m_operand_type != replacement_type.value_type()) {
      std::stringstream ss;
      ss << "Cannot chain types, because the view's storage type, "
         << m_operand_type;
      ss << ", does not match the replacement's value type, "
         << replacement_type.value_type();
      throw dynd::type_error(ss.str());
    }
    return type(new view_type(m_value_type, replacement_type), false);
  }
}

size_t ndt::view_type::make_operand_to_value_assignment_kernel(
    ...
    const eval::eval_context *DYND_UNUSED(ectx)) const
{
  return ::make_pod_typed_data_assignment_kernel(
      ...
      std::min(m_value_type.get_data_alignment(),
               m_operand_type.get_data_alignment()),
      kernreq);
}

size_t ndt::view_type::make_value_to_operand_assignment_kernel(
    void *ckb, intptr_t ckb_offset, const char *DYND_UNUSED(dst_arrmeta),
    const char *DYND_UNUSED(src_arrmeta), kernel_request_t kernreq,
    const eval::eval_context *DYND_UNUSED(ectx)) const
{
