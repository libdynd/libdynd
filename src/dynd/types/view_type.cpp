      ...
      m_value_type(value_type), m_operand_type(operand_type)
{
  if (value_type.get_data_size() != operand_type.value_type().get_data_size()) {
    std::stringstream ss;
    ss << "view_type: Cannot view " << operand_type.value_type() << " as " << value_type
...
void ndt::view_type::print_data(std::ostream &o, const char *arrmeta, const char *data) const
{
  ...
  if (m_operand_type.get_type_id() == fixed_bytes_type_id) {
    ...
    default: {
      ...
      buffer = (char *)(((uintptr_t)buffer + (uintptr_t)m_value_type.get_data_alignment() - 1) &
...
void ndt::view_type::get_shape(intptr_t ndim, intptr_t i, intptr_t *out_shape, const char *DYND_UNUSED(arrmeta),
...
bool ndt::view_type::is_lossless_assignment(const type &dst_tp, const type &src_tp) const
{
...
ndt::type ndt::view_type::with_replaced_storage_type(const type &replacement_type) const
{
  if (m_operand_type.get_kind() == expr_kind) {
    return type(new view_type(m_value_type,
                              m_operand_type.extended<base_expr_type>()->with_replaced_storage_type(replacement_type)),
                false);
  }
  else {
...
size_t ndt::view_type::make_operand_to_value_assignment_kernel(nd::kernel_builder *ckb, intptr_t ckb_offset,
...
size_t ndt::view_type::make_value_to_operand_assignment_kernel(nd::kernel_builder *ckb, intptr_t ckb_offset,
                                                               const char *DYND_UNUSED(dst_arrmeta),
                                                               const char *DYND_UNUSED(src_arrmeta),
                                                               kernel_request_t kernreq,
                                                               const eval::eval_context *DYND_UNUSED(ectx)) const
