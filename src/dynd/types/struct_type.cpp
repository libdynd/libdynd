...
namespace nd {
  ...
  struct get_array_field_kernel : nd::base_kernel<get_array_field_kernel> {
    ...
    static void instantiate(char *static_data, char *DYND_UNUSED(data), nd::kernel_builder *ckb, intptr_t DYND_UNUSED(ckb_offset),
                            ...
                            const std::map<std::string, ndt::type> &DYND_UNUSED(tp_vars))
    {
      get_array_field_kernel::make(ckb, kernreq, kwds[0], *reinterpret_cast<intptr_t *>(static_data));
    }
