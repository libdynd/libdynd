...
strided_dim_type::strided_dim_type(const ndt::type& element_dtype)
    : base_uniform_dim_type(strided_dim_type_id, element_dtype, 0, element_dtype.get_data_alignment(),
...
bool strided_dim_type::is_unique_data_owner(const char *metadata) const
{
    if (m_element_dtype.is_builtin()) {
        ...
        return m_element_dtype.extended()->is_unique_data_owner(metadata + sizeof(strided_dim_type_metadata));
    }
}
...
{
    return ndt::type(new strided_dim_type(m_element_dtype.get_canonical_type()), false);
}
    ...
    if (nindices == 0) {
        ...
        if (remove_dimension) {
            ...
            if (!m_element_dtype.is_builtin()) {
                ...
                offset += m_element_dtype.extended()->apply_linear_index(nindices - 1, indices + 1,
                                metadata + sizeof(strided_dim_type_metadata),
                                result_edtype->m_element_dtype, out_metadata + sizeof(strided_dim_type_metadata),
                                embedded_reference, current_i + 1, root_dt,
                                false, NULL, NULL);
            }
            return offset;
...
void strided_dim_type::get_strides(size_t i, intptr_t *out_strides, const char *metadata) const
{
    ...
    // Process the later shape values
    if (!m_element_dtype.is_builtin()) {
        m_element_dtype.extended()->get_strides(i+1, out_strides, metadata + sizeof(strided_dim_type_metadata));
    }
}
...
bool strided_dim_type::is_lossless_assignment(const ndt::type& dst_dt, const ndt::type& src_dt) const
{
...
void strided_dim_type::metadata_default_construct(char *metadata, size_t ndim, const intptr_t* shape) const
{
    ...
    if (!m_element_dtype.is_builtin()) {
        m_element_dtype.extended()->metadata_default_construct(metadata + sizeof(strided_dim_type_metadata), ndim-1, shape+1);
    }
}
...
void strided_dim_type::metadata_copy_construct(char *dst_metadata, const char *src_metadata, memory_block_data *embedded_reference) const
{
    ...
    if (!m_element_dtype.is_builtin()) {
        m_element_dtype.extended()->metadata_copy_construct(dst_metadata + sizeof(strided_dim_type_metadata),
...
{
    if (!m_element_dtype.is_builtin()) {
        m_element_dtype.extended()->metadata_finalize_buffers(metadata + sizeof(strided_dim_type_metadata));
    }
}
...
{
    if (!m_element_dtype.is_builtin()) {
        m_element_dtype.extended()->metadata_destruct(metadata + sizeof(strided_dim_type_metadata));
    }
}
...
void strided_dim_type::metadata_debug_print(const char *metadata, std::ostream& o, const std::string& indent) const
{
    ...
    if (!m_element_dtype.is_builtin()) {
        m_element_dtype.extended()->metadata_debug_print(metadata + sizeof(strided_dim_type_metadata), o, indent + " ");
    }
}
...
size_t strided_dim_type::get_iterdata_size(size_t ndim) const
{
    if (ndim == 0) {
        ...
        return m_element_dtype.get_iterdata_size(ndim - 1) + sizeof(strided_dim_type_iterdata);
    }
}
...
size_t strided_dim_type::iterdata_construct(iterdata_common *iterdata, const char **inout_metadata, size_t ndim, const intptr_t* shape, ndt::type& out_uniform_dtype) const
{
    const strided_dim_type_metadata *md = reinterpret_cast<const strided_dim_type_metadata *>(*inout_metadata);
    *inout_metadata += sizeof(strided_dim_type_metadata);
    size_t inner_size = 0;
    ...
    if (this == dst_dt.extended()) {
        ...
        if (src_dt.get_undim() < dst_dt.get_undim()) {
            ...
            return ::make_assignment_kernel(out, offset_out + sizeof(strided_assign_kernel_extra),
                            m_element_dtype, dst_metadata + sizeof(strided_dim_type_metadata),
                            src_dt, src_metadata,
                ...
                const ndt::type& src_dt, const char *src_metadata) const
{
    ...
    if (get_undim() > src_dt.get_undim()) {
        ...
        if (m_element_dtype.get_type_id() == strided_dim_type_id) {
            const strided_dim_type *sdd = static_cast<const strided_dim_type *>(m_element_dtype.extended());
    ...
    for (size_t i = 0; i < ndim; ++i) {
        ...
        switch (last_src_dtype.get_type_id()) {
            case fixed_dim_type_id: {
                const fixed_dim_type *fdd = static_cast<const fixed_dim_type *>(last_src_dtype.extended());
                stride = fdd->get_fixed_stride();
                last_src_dtype = fdd->get_element_type();
                break;
            }
            case strided_dim_type_id: {
                const strided_dim_type *sdd = static_cast<const strided_dim_type *>(last_src_dtype.extended());
