            ...
            m_property_index(property_index)
{
    if (!operand_type.value_type().is_builtin()) {
        ...
        m_value_type = m_operand_type.value_type().extended()->get_elwise_property_type(
        ...
        m_value_type = get_builtin_type_elwise_property_type(
                        operand_type.value_type().get_type_id(),
                        m_property_index, m_readable, m_writable);
    }
...
property_type::property_type(const ndt::type& value_type, const ndt::type& operand_type,
            ...
            m_property_index(property_index)
{
    if (m_value_type.get_kind() == expression_kind) {
        stringstream ss;
        ss << "property_type: The destination dtype " << m_value_type;
    ...
    if (!value_type.is_builtin()) {
        ...
        property_dt = m_value_type.extended()->get_elwise_property_type(
...
bool property_type::is_lossless_assignment(const ndt::type& dst_dt, const ndt::type& src_dt) const
{
...
ndt::type property_type::with_replaced_storage_type(const ndt::type& replacement_type) const
{
    if (m_operand_type.get_kind() == expression_kind) {
        ...
        if (!m_reversed_property) {
            ...
            return ndt::type(new property_type(m_value_type, replacement_type, m_property_name), false);
        }
    }
}
