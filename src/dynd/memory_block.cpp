...
intrusive_ptr<nd::base_memory_block> nd::make_fixed_size_pod_memory_block(intptr_t size_bytes, intptr_t alignment,
                                                                          char **out_datapointer) {
  // Calculate the aligned starting point for the data
  intptr_t start =
      (intptr_t)(((uintptr_t)sizeof(base_memory_block) + (uintptr_t)(alignment - 1)) & ~((uintptr_t)(alignment - 1)));
  // Allocate it
  ...
  *out_datapointer = reinterpret_cast<char *>(result) + start;
  ...
  return intrusive_ptr<base_memory_block>(result, false);
}
