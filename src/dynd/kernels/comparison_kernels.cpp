                                    ...
                                    const eval::eval_context *ectx)
{
  if (src0_dt.is_builtin()) {
    if (src1_dt.is_builtin()) {
      return make_builtin_type_comparison_kernel(
          ckb, ckb_offset, src0_dt.get_type_id(), src1_dt.get_type_id(),
          comptype);
    } else {
    ...
    return src0_dt.extended()->make_comparison_kernel(
        ckb, ckb_offset, src0_dt, src0_arrmeta, src1_dt, src1_arrmeta, comptype,
        ectx);
  }
}
...
#define SRC0_TYPE_LEVEL(src0_type)                                             \
  {                                                                            \
    INNER_LEVEL(src0_type, bool1), INNER_LEVEL(src0_type, int8_t),             \
        ...
        INNER_LEVEL(src0_type, int64_t), INNER_LEVEL(src0_type, dynd_int128),  \
    ...
    SRC0_TYPE_LEVEL(int32_t), SRC0_TYPE_LEVEL(int64_t),
    SRC0_TYPE_LEVEL(dynd_int128), SRC0_TYPE_LEVEL(uint8_t),
    SRC0_TYPE_LEVEL(uint16_t), SRC0_TYPE_LEVEL(uint32_t),
    SRC0_TYPE_LEVEL(uint64_t), SRC0_TYPE_LEVEL(dynd_uint128),
...
size_t dynd::make_builtin_type_comparison_kernel(void *ckb, intptr_t ckb_offset,
