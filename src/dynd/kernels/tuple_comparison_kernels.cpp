...
struct tuple_compare_sorting_less_matching_arrmeta_kernel {
  ...
  static void sorting_less(ckernel_prefix *extra, char *dst, char *const *src)
  {
    ...
    for (size_t i = 0; i != field_count; ++i) {
      ...
      kernel_single_t opchild = sorting_less_kdp->get_function<kernel_single_t>();
...
struct tuple_compare_sorting_less_diff_arrmeta_kernel {
  ...
  static void sorting_less(ckernel_prefix *extra, char *dst, char *const *src)
  {
    ...
    for (size_t i = 0; i != field_count; ++i) {
      ...
      kernel_single_t opchild = src0_sorting_less_src1->get_function<kernel_single_t>();
      // if (src0.field_i < src1.field_i) return true
      ...
      opchild = src1_sorting_less_src0->get_function<kernel_single_t>();
      // if (src1.field_i < src0.field_i) return false
  ...
  if (comptype == comparison_type_sorting_less) {
    if (src0_arrmeta == src1_arrmeta || src_tp.get_arrmeta_size() == 0 ||
      ...
      for (size_t i = 0; i != field_count; ++i) {
        ...
        // move the memory.
        ckb->reserve(ckb_offset + sizeof(ckernel_prefix));
        e = ckb->get_at<tuple_compare_sorting_less_matching_arrmeta_kernel>(root_ckb_offset);
    ...
    //    reinterpret_cast<kernel_builder *>(ckb)
    ...
    if (comptype == comparison_type_equal) {
      ...
      nd::equal_kernel<tuple_id, tuple_id>::instantiate(
      ...
      std::map<std::string, ndt::type> tp_vars;
      const char *src_arrmeta[2] = {src0_arrmeta, src1_arrmeta};
      nd::not_equal_kernel<tuple_id, tuple_id>::instantiate(
          NULL, NULL, ckb, ndt::make_type<bool1>(), NULL, 2, &src_tp, src_arrmeta,
          kernel_request_host | kernel_request_single, 0, NULL, tp_vars);
      ckb_offset = ckb->m_size;
      return;
