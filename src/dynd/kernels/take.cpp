    ...
    const std::map<nd::string, ndt::type> &DYND_UNUSED(tp_vars))
{
  ...
  return make_assignment_kernel(NULL, NULL, ckb, ckb_offset, dst_el_tp,
                                dst_el_meta, src0_el_tp, src0_el_meta,
    ...
    const nd::array &kwds, const std::map<nd::string, ndt::type> &tp_vars)
{
  ...
  if (mask_el_tp.get_type_id() == bool_type_id) {
    return nd::masked_take_ck::instantiate(
        af_self, af_tp, NULL, 0, NULL, ckb, ckb_offset, dst_tp, dst_arrmeta,
...
void nd::take_ck::resolve_dst_type(
    const arrfunc_type_data *DYND_UNUSED(af_self),
    const ndt::arrfunc_type *af_tp, const char *DYND_UNUSED(static_data),
