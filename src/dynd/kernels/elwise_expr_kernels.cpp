                ...
                const expr_kernel_generator *elwise_handler)
{
    ...
    if (dst_dt.get_type_id() == strided_dim_type_id) {
        const strided_dim_type *sdd = static_cast<const strided_dim_type *>(dst_dt.extended());
        ...
        const fixed_dim_type *fdd = static_cast<const fixed_dim_type *>(dst_dt.extended());
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (src_dt[i].get_undim() < undim) {
            ...
            const strided_dim_type *sdd = static_cast<const strided_dim_type *>(src_dt[i].extended());
            ...
            const fixed_dim_type *fdd = static_cast<const fixed_dim_type *>(src_dt[i].extended());
            // Check for a broadcasting error
                ...
                const expr_kernel_generator *elwise_handler)
{
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (src_dt[i].get_undim() < undim) {
            ...
            const var_dim_type *vdd = static_cast<const var_dim_type *>(src_dt[i].extended());
            ...
            e->is_src_var[i] = true;
            src_child_metadata[i] = src_metadata[i] + sizeof(var_dim_type_metadata);
                ...
                const expr_kernel_generator *elwise_handler)
{
    ...
    const var_dim_type *dst_vdd = static_cast<const var_dim_type *>(dst_dt.extended());
    ...
    dst_child_metadata = dst_metadata + sizeof(var_dim_type_metadata);
