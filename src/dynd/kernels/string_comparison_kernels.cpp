...
size_t dynd::make_fixed_string_comparison_kernel(
    void *ckb, intptr_t ckb_offset, size_t string_size,
...
struct string_compare_kernel {
  static void less(char *dst, char *const *src,
  ...
  static void equal(char *dst, char *const *src,
                    ckernel_prefix *DYND_UNUSED(self))
  {
    const string_type_data *da =
        reinterpret_cast<const string_type_data *>(src[0]);
...
#define DYND_STRING_COMPARISON_TABLE_TYPE_LEVEL(type)                          \
  {                                                                            \
    string_compare_kernel<type>::less, string_compare_kernel<type>::less,      \
...
size_t dynd::make_string_comparison_kernel(
    void *ckb, intptr_t ckb_offset, string_encoding_t encoding,
    ...
    const eval::eval_context *ectx)
{
  ...
  return make_comparison_kernel(
      ckb, ckb_offset, ndt::convert_type::make(sdt, src0_dt), src0_arrmeta,
