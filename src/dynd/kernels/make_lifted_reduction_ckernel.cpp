...
struct strided_initial_reduction_kernel_extra
  ...
  static void single_first(ckernel_prefix *extra, char *dst, char *const *src)
  {
...
struct strided_inner_reduction_kernel_extra
                                      ...
                                      char *const *src)
  {
    ...
    expr_strided_t opchild_reduce =
        echild_reduce->get_function<expr_strided_t>();
    opchild_ident(echild_ident, dst, const_cast<char *const *>(&e->ident_data));
    // All the followup calls to accumulate at the "dst" address
                                       ...
                                       const intptr_t *src_stride, size_t count)
  {
    ...
    if (dst_stride == 0) {
      // With a zero stride, we initialize "dst" once, then do many
      // accumulations
      opchild_ident(echild_ident, dst, const_cast<char *const *>(&ident_data));
...
struct strided_inner_broadcast_kernel_extra
  ...
  static void single_first(ckernel_prefix *extra, char *dst, char *const *src)
  {
    ...
    opchild_dst_init(echild_dst_init, dst, e->dst_stride, src, &e->src_stride,
                     e->size);
  }
