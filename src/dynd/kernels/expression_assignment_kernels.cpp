...
struct buffered_kernel_extra {
  ...
  void init(const ndt::type &buffer_tp_, kernel_request_t kernreq)
  {
    ...
    if (!buffer_tp_.is_builtin()) {
      ...
      buffer_data_size =
          inc_to_alignment(element_count * buffer_stride, sizeof(void *));
    } else {
      // Make sure the buffer data size is pointer size-aligned
  ...
  static void single(char *dst, char *const *src, ckernel_prefix *extra)
  {
    ...
    echild_first =
        reinterpret_cast<ckernel_prefix *>(eraw + e->first_kernel_offset);
    echild_second =
        reinterpret_cast<ckernel_prefix *>(eraw + e->second_kernel_offset);
  ...
  if (dst_tp.get_kind() == expr_kind) {
    ...
    if (src_tp == dst_bed->get_value_type()) {
      ...
      if (opdt.get_kind() != expr_kind) {
        ...
        return ::make_assignment_kernel(
            NULL, NULL, ckb, ckb_offset, opdt, dst_arrmeta, buffer_tp,
      ...
      ckb_offset = ::make_assignment_kernel(
          NULL, NULL, ckb, ckb_offset, buffer_tp, e->buffer_arrmeta, src_tp,
    ...
    if (dst_tp == src_bed->get_value_type()) {
      ...
      if (opdt.get_kind() != expr_kind) {
        ...
        e->first_kernel_offset = ckb_offset - root_ckb_offset;
        ckb_offset = ::make_assignment_kernel(
            NULL, NULL, ckb, ckb_offset, buffer_tp, e->buffer_arrmeta, opdt,
            src_arrmeta, kernreq, ectx, nd::array());
        // Allocate the buffer data
        ckb_offset =
            inc_to_alignment(ckb_offset, buffer_tp.get_data_alignment());
