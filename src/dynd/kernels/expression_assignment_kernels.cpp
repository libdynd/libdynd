        ...
        if (src_tp == dst_bed->get_value_type()) {
            ...
            if (opdt.get_kind() != expr_kind) {
                ...
                e = reinterpret_cast<ckernel_builder<kernel_request_host> *>(
                        ckb)->get_at<buffered_kernel_extra>(root_ckb_offset);
            ...
            // Construct the first kernel (src -> buffer)
            e->first_kernel_offset = ckb_offset - root_ckb_offset;
            ckb_offset = ::make_assignment_kernel(NULL, NULL, ckb, ckb_offset,
                            buffer_tp, e->buffer_arrmeta,
                            src_tp, src_arrmeta,
                            kernreq, ectx, nd::array());
        ...
        if (dst_tp == src_bed->get_value_type()) {
            ...
            if (opdt.get_kind() != expr_kind) {
                ...
                ckb_offset = ::make_assignment_kernel(NULL, NULL,
                    ...
                    e->buffer_arrmeta, opdt, src_arrmeta, kernreq, ectx, nd::array());
                // Allocate the buffer data
                ckb_offset = inc_to_alignment(ckb_offset, buffer_tp.get_data_alignment());
            ...
            // Construct the first kernel (src -> buffer)
            e->first_kernel_offset = ckb_offset - root_ckb_offset;
            ckb_offset = ::make_assignment_kernel(NULL, NULL, ckb, ckb_offset, buffer_tp,
            ...
            return ::make_assignment_kernel(NULL, NULL,
                ...
                buffer_tp, e->buffer_arrmeta, kernreq, ectx, nd::array());
        }
    }
}
