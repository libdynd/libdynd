...
struct tuple_unary_op_ck
    : nd::base_kernel<tuple_unary_op_ck, kernel_request_host, 1> {
    ...
    const eval::eval_context *ectx)
{
  ...
  for (intptr_t i = 0; i < field_count; ++i) {
    ...
    ckb_offset = af->instantiate(af, af_tp, NULL, 0, NULL, ckb, ckb_offset, dst_tp[i],
    ...
    kernel_request_t kernreq, const eval::eval_context *ectx)
{
  ...
  for (intptr_t i = 0; i < field_count; ++i) {
    ...
    ckb_offset = af[i]->instantiate(
        af[i], af_tp[i], NULL, 0, NULL, ckb, ckb_offset, dst_tp[i], dst_arrmeta[i], 1,
