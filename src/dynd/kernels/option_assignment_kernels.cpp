    ...
    const nd::array &kwds, const std::map<nd::string, ndt::type> &tp_vars)
{
  ...
  const ndt::type &dst_val_tp = dst_tp.extended<option_type>()->get_value_type();
  ...
  ckb_offset =
      af->instantiate(af, af_tp, NULL, ckb, ckb_offset, ndt::make_type<dynd_bool>(),
                      NULL, nsrc, src_tp, src_arrmeta, kernreq, ectx, kwds, tp_vars);
  ...
  ckb_offset =
      make_assignment_kernel(NULL, NULL, ckb, ckb_offset, dst_val_tp, dst_arrmeta,
                             src_val_tp, src_arrmeta[0], kernreq, ectx, nd::array());
  return ckb_offset;
}
    ...
    const nd::array &kwds, const std::map<nd::string, ndt::type> &tp_vars)
{
  ...
  type_id_t tid = dst_tp.extended<option_type>()->get_value_type().get_type_id();
  switch (tid) {
  case bool_type_id: {
  ...
  case string_type_id: {
    ...
    return ::make_assignment_kernel(NULL, NULL,
        ckb, ckb_offset, dst_tp.extended<option_type>()->get_value_type(),
        dst_arrmeta, src_tp[0], src_arrmeta[0], kernreq, ectx, nd::array());
  }
  default:
    break;
  }
  ...
  self = reinterpret_cast<ckernel_builder<kernel_request_host> *>(ckb)->get_at<string_to_option_tp_ck>(root_ckb_offset);
    ...
    const nd::array &kwds, const std::map<nd::string, ndt::type> &tp_vars)
{
  ...
  return instantiate_option_to_option_assignment_kernel(
      self, af_tp, NULL, ckb, ckb_offset, dst_tp, dst_arrmeta, nsrc, &src_tp_as_option,
    ...
    const nd::array &DYND_UNUSED(kwds), const std::map<nd::string, ndt::type> &DYND_UNUSED(tp_vars))
{
  ...
  ndt::type val_src_tp =
      src_tp[0].get_type_id() == option_type_id
          ? src_tp[0].extended<option_type>()->get_value_type()
          : src_tp[0];
  return ::make_assignment_kernel(NULL, NULL, ckb, ckb_offset, val_dst_tp, dst_arrmeta,
                                  val_src_tp, src_arrmeta[0], kernreq, ectx, nd::array());
}
    ...
    kernel_request_t kernreq, const eval::eval_context *ectx)
{
  ...
  for (intptr_t i = 0; i < size; ++i, ++af_tp, ++af) {
    ...
    if ((*af_tp)->get_pos_type(0).match(src_tp, typevars) &&
      ...
      return af->instantiate(af, *af_tp, NULL, ckb, ckb_offset, dst_tp, dst_arrmeta,
