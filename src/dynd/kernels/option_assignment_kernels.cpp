...
struct option_callable_list {
  ...
  option_callable_list()
  {
    ...
    af[i].instantiate =
        &nd::assignment_kernel<option_type_id, string_type_id>::instantiate;
    ++i;
    af_tp[i] = ndt::type("(?T) -> ?S");
    af[i].instantiate = &dynd::nd::assignment_kernel<
                             option_type_id, option_type_id>::instantiate;
    ++i;
    af_tp[i] = ndt::type("(?T) -> S");
    ...
    af[i].instantiate =
        &nd::assignment_kernel<option_type_id, float32_type_id>::instantiate;
    ++i;
    af_tp[i] = ndt::type("(float64) -> ?S");
    af[i].instantiate =
        &nd::assignment_kernel<option_type_id, float64_type_id>::instantiate;
    ++i;
    af_tp[i] = ndt::type("(T) -> S");
  ...
  const nd::base_callable *get() const
  {
    return af;
  }
