...
dispatcher<nd::callable> make_comparison_children()
{
  ...
  nd::callable self = nd::functional::call<Func>(ndt::type("(Any, Any) -> Any"));
  ...
  for (type_id_t i0 : i2a<numeric_ids>()) {
    for (type_id_t i1 : i2a<dim_ids>()) {
      ...
      dispatcher.insert({{i0, i1}, nd::functional::elwise(child_tp, self)});
    }
  }
  ...
  dispatcher.insert({{option_id, option_id}, nd::make_callable<nd::option_comparison_callable<Func, true, true>>()});

  for (type_id_t dim_tp_id : i2a<dim_ids>()) {
    dispatcher.insert({{dim_tp_id, option_id}, nd::functional::elwise(self)});
    dispatcher.insert({{option_id, dim_tp_id}, nd::functional::elwise(self)});
  }
