...
const char *dynd_exception::message() const throw() { return m_message.c_str(); }
const char *dynd_exception::what() const throw() { return m_what.c_str(); }
dynd_exception::~dynd_exception() throw() {}
...
too_many_indices::~too_many_indices() throw() {}
...
index_out_of_bounds::index_out_of_bounds(intptr_t i, size_t axis, const std::vector<intptr_t> &shape)
    : dynd_exception("index out of bounds",
                     index_out_of_bounds_message(i, axis, (int)shape.size(), shape.empty() ? NULL : &shape[0]))
{
}
...
index_out_of_bounds::~index_out_of_bounds() throw() {}
...
axis_out_of_bounds::~axis_out_of_bounds() throw() {}
...
irange_out_of_bounds::irange_out_of_bounds(const irange &i, size_t axis, const std::vector<intptr_t> &shape)
    : dynd_exception("irange out of bounds",
                     irange_out_of_bounds_message(i, axis, (int)shape.size(), shape.empty() ? NULL : &shape[0]))
{
}
...
irange_out_of_bounds::~irange_out_of_bounds() throw() {}
...
invalid_id::invalid_id(int type_id) : type_error("invalid type id", invalid_id_message(type_id))
{
...
string_decode_error::string_decode_error(const char *begin, const char *end, string_encoding_t encoding)
    : dynd_exception("string decode error", string_decode_error_message(begin, end, encoding)), m_bytes(begin, end),
      m_encoding(encoding)
{
}

const std::string &string_decode_error::bytes() const { return m_bytes; }
string_encoding_t string_decode_error::encoding() const { return m_encoding; }

string_decode_error::~string_decode_error() throw() {}
...
string_encode_error::string_encode_error(uint32_t cp, string_encoding_t encoding)
    : dynd_exception("string encode error", string_encode_error_message(cp, encoding)), m_cp(cp), m_encoding(encoding)
{
}

uint32_t string_encode_error::cp() const { return m_cp; }
string_encoding_t string_encode_error::encoding() const { return m_encoding; }
string_encode_error::~string_encode_error() throw() {}

inline std::string not_comparable_error_message(const ndt::type &lhs, const ndt::type &rhs, comparison_type_t comptype)
{
  stringstream ss;
...
not_comparable_error::not_comparable_error(const ndt::type &lhs, const ndt::type &rhs, comparison_type_t comptype)
    : dynd_exception("not comparable error", not_comparable_error_message(lhs, rhs, comptype))
{
}

not_comparable_error::~not_comparable_error() throw() {}

zero_division_error::~zero_division_error() throw() {}
