...
{
  return (value & 0xf0u) ? ((value & 0xc0u) ? ((value & 0x80u) ? 0u : 1u) : ((value & 0x20u) ? 2u : 3u))
                         : ((value & 0x0cu) ? ((value & 0x08u) ? 4u : 5u) : ((value & 0x02u) ? 6u : 7u));
}

inline uint8_t leading_zerobits(uint16_t value)
{
  return (value & 0xff00u) ? (leading_zerobits((uint8_t)(value >> 8))) : (leading_zerobits((uint8_t)value) + 8u);
}
...
{
  return (value & 0xffffffff00000000ULL) ? (leading_zerobits((uint32_t)(value >> 32)))
...
dynd::float128::float128(const float16 &value) { *this = float128(double(value)); }
