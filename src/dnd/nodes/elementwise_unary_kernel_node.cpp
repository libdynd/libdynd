...
ndarray_node_ptr dnd::elementwise_unary_kernel_node::as_dtype(const dtype& dt,
                    dnd::assign_error_mode errmode, bool allow_in_place)
{
    if (allow_in_place) {
        ...
        ndarray_node_ptr result(
                make_elementwise_unary_kernel_node_copy_kernel(make_convert_dtype(dt, m_dtype, errmode),
                                m_opnode, m_kernel));
        return result;
    }
}

ndarray_node_ptr dnd::elementwise_unary_kernel_node::apply_linear_index(
                ...
                bool allow_in_place)
{
    if (allow_in_place) {
        ...
        return ndarray_node_ptr(
                    make_elementwise_unary_kernel_node_copy_kernel(m_dtype, m_opnode, m_kernel));
    }
}

void dnd::elementwise_unary_kernel_node::get_unary_operation(intptr_t dst_fixedstride, intptr_t src_fixedstride,
...
ndarray_node_ptr dnd::make_elementwise_unary_kernel_node_copy_kernel(const dtype& dt, const ndarray_node_ptr& opnode,
                                            const unary_specialization_kernel_instance& kernel)
{
    char *node_memory = NULL;
    ndarray_node_ptr result(make_uninitialized_ndarray_node_memory_block(sizeof(elementwise_unary_kernel_node), &node_memory));

    // Placement new
    new (node_memory) elementwise_unary_kernel_node(
                        dt, opnode, kernel);

    return DND_MOVE(result);
}

ndarray_node_ptr dnd::make_elementwise_unary_kernel_node_steal_kernel(const dtype& dt, const ndarray_node_ptr& opnode,
                                            unary_specialization_kernel_instance& kernel)
{
    ...
    // Placement new
    elementwise_unary_kernel_node *ukn = new (node_memory) elementwise_unary_kernel_node(
                        dt, opnode);
