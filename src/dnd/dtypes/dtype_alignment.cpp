...
dtype dnd::make_unaligned_dtype(const dtype& value_dtype)
{
    if (value_dtype.alignment() > 1) {
        ...
        if (value_dtype.kind() != expression_kind) {
            return make_view_dtype(value_dtype, make_bytes_dtype(value_dtype.element_size(), 1));
        } else {
            const dtype& sdt = value_dtype.storage_dtype();
            return dtype(value_dtype.extended()->with_replaced_storage_dtype(make_view_dtype(sdt, make_bytes_dtype(sdt.element_size(), 1))));
        }
    } else {
        return value_dtype;
    }
