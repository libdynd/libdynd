...
dtype dnd::convert_dtype::apply_linear_index(int nindices, const irange *indices, int current_i, const dtype& root_dt) const
{
    if (nindices == 0) {
        return dtype(this);
    } else {
...
void dnd::convert_dtype::get_shape(int i, std::vector<intptr_t>& out_shape) const
{
